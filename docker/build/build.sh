...
prepare_build () {
  ...
  if [ ! -f ./otoroshi-dist.zip ]; then
    ...
    sbt assembly
    cd $LOCATION
    cp ../../otoroshi/target/universal/otoroshi-1.5.3.zip ./otoroshi-dist.zip
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar ./otoroshi.jar
  fi
  unzip otoroshi-dist.zip
  mv otoroshi-1.5.3 otoroshi
...
case "${1}" in
  ...
  build-and-push-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.5.3-$NBR"
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar otoroshi.jar
    docker build --no-cache -f ./Dockerfile-jdk11-jar -t otoroshi-jdk11 .
    docker tag otoroshi-jdk11 "maif/otoroshi:1.5.3-$NBR"
    docker tag otoroshi-jdk11 "maif/otoroshi:dev"
    cleanup
    docker push "maif/otoroshi:1.5.3-$NBR"
    docker push "maif/otoroshi:dev"
    ;;
  build-and-push-local)
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar otoroshi.jar
    docker build --no-cache -f ./Dockerfile-jdk11-jar -t otoroshi-jdk11 .
    docker tag otoroshi-jdk11 "registry.oto.tools:5000/maif/otoroshi:1.5.3-local"
    cleanup
    docker push "registry.oto.tools:5000/maif/otoroshi:1.5.3-local"
    ;;
  build-snapshot)
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar otoroshi.jar
    docker build --no-cache -f ./Dockerfile-jdk18-jar -t otoroshi-jdk18 .

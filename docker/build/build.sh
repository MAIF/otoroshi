...
prepare_build () {
  ...
  if [ ! -f ./otoroshi-dist.zip ]; then
    ...
    sbt assembly
    cd $LOCATION
    cp ../../otoroshi/target/universal/otoroshi-1.4.0.zip ./otoroshi-dist.zip
...
# sh ./build.sh build-all 1.3.2-dev-$(date +%s)
...
case "${1}" in
  ...
  build-and-push-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.3.2-dev-$NBR"
    cp ../../otoroshi/target/universal/otoroshi-1.3.2-dev.zip otoroshi-dist.zip
    prepare_build
    docker build --no-cache -t otoroshi .
    docker tag otoroshi "maif/otoroshi:1.3.2-dev-$NBR"
    cleanup
    docker push "maif/otoroshi:1.3.2-dev-$NBR"
    ;;
  build-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.3.2-dev-$NBR"

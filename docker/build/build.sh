...
prepare_build () {
  ...
  if [ ! -f ./otoroshi-dist.zip ]; then
    ...
    sbt assembly
    cd $LOCATION
    cp ../../otoroshi/target/universal/otoroshi-1.4.7-dev.zip ./otoroshi-dist.zip
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar ./otoroshi.jar
  fi
  unzip otoroshi-dist.zip
  mv otoroshi-1.4.7-dev otoroshi
...
case "${1}" in
  ...
  build-and-push-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.4.7-dev-$NBR"
    cp ../../otoroshi/target/universal/otoroshi-1.4.7-dev.zip otoroshi-dist.zip
    prepare_build
    docker build --no-cache -t otoroshi .
    docker tag otoroshi "maif/otoroshi:1.4.7-dev-$NBR"
    cleanup
    docker push "maif/otoroshi:1.4.7-dev-$NBR"
    ;;
  build-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.4.7-dev-$NBR"

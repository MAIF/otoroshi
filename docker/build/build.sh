...
prepare_build () {
  ...
  if [ ! -f ./otoroshi-dist.zip ]; then
    ...
    sbt assembly
    cd $LOCATION
    cp ../../otoroshi/target/universal/otoroshi-1.5.0-alpha.14.zip ./otoroshi-dist.zip
    cp ../../otoroshi/target/scala-2.12/otoroshi.jar ./otoroshi.jar
  fi
  unzip otoroshi-dist.zip
  mv otoroshi-1.5.0-alpha.14 otoroshi
...
case "${1}" in
  ...
  build-and-push-snapshot)
    NBR=`date +%s`
    echo "Will build version 1.5.0-alpha.14-$NBR"
    cp ../../otoroshi/target/universal/otoroshi-1.5.0-alpha.14.zip otoroshi-dist.zip
    prepare_build
    docker build --no-cache -f ./Dockerfile-jdk11 -t otoroshi-jdk11 .
    docker tag otoroshi-jdk11 "maif/otoroshi:1.5.0-alpha.14-$NBR"
    docker tag otoroshi-jdk11 "maif/otoroshi:dev"
    cleanup
    docker push "maif/otoroshi:1.5.0-alpha.14-$NBR"

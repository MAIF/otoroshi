package io.otoroshi.common.wasm.impl

import akka.stream.OverflowStrategy
import akka.stream.scaladsl._
import com.codahale.metrics.UniformReservoir
import io.otoroshi.common.wasm.scaladsl.CacheableWasmScript.CachedWasmScript
...
case class WasmVmPoolAction(promise: Promise[WasmVmImpl], options: WasmVmInitOptions) {
  private[wasm] def provideVm(vm: WasmVmImpl): Unit = promise.trySuccess(vm)
...
class WasmVmPoolImpl(stableId: => String, optConfig: => Option[WasmConfiguration], val ic: WasmIntegrationContext) extends WasmVmPool {
  ...
  private[wasm] def release(vm: WasmVmImpl): Unit = synchronized {
    availableVms.synchronized {
      availableVms.offer(vm)
      inUseVms.remove(vm)
    }
  ...
  private[wasm] def ignore(vm: WasmVmImpl): Unit = synchronized {
    availableVms.synchronized {
      inUseVms.remove(vm)
    }
  }
  ...
  // do not consider the vm anymore for more work (the vm is being dropped for some reason)
  private[wasm] def clear(vm: WasmVmImpl): Unit = synchronized {
    availableVms.synchronized {
      availableVms.remove(vm)
    }
  }

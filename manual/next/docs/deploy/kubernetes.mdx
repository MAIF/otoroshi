...
{`bases:
- github.com/MAIF/otoroshi/kubernetes/kustomize/overlays/simple/?ref=v${version}`}
</CodeBlock>
...
The chart lives at [`kubernetes/helm/otoroshi/`](https://github.com/MAIF/otoroshi/tree/master/kubernetes/helm/otoroshi). It ships:

- a single-pod **standalone mode** (default) or a **Leader / Worker cluster mode** (`cluster.enabled=true`);
- a bundled **Redis subchart** that you can disable to bring your own (`redis.deploy=false`, set `env.redisURL`);
- the Otoroshi **CRDs** and Kubernetes **Gateway API** integration (opt-in via `gatewayApi.enabled=true`);
- optional **PodDisruptionBudget**, **NetworkPolicy**, **HPA**, validating / sidecar-injector **admission webhooks**, and a Helm **smoke test**.
...
Admin credentials (`password`, `clientId`, `clientSecret`, `OTOROSHI_SECRET`) are auto-generated on first install and stored in a Kubernetes `Secret`. They are preserved across `helm upgrade` via the Helm `lookup` function — read them back with:

```sh
kubectl -n <ns> get secret otoroshi-admin-secret -o jsonpath='{.data.password}' | base64 -d
```

Bring your own Secret instead (recommended for GitOps):

```sh
kubectl create secret generic my-otoroshi-creds \
  --from-literal=password=$(openssl rand -hex 24) \
  --from-literal=clientId=admin-api-apikey-id \
  --from-literal=clientSecret=$(openssl rand -hex 32) \
  --from-literal=otoroshiSecret=$(openssl rand -hex 32)

helm install my-otoroshi otoroshi/otoroshi --set env.existingSecret=my-otoroshi-creds
```

Common overrides:

```sh
# External / managed Redis (skip bundled subchart)
helm install my-otoroshi otoroshi/otoroshi \
  --set redis.deploy=false \
  --set env.redisURL=redis://redis.example.com:6379/0 \
  --set env.redisPasswordExistingSecret.name=my-redis-creds \
  --set env.redisPasswordExistingSecret.key=password

# Otoroshi cluster (Leader + Worker)
helm install my-otoroshi otoroshi/otoroshi \
  --set cluster.enabled=true \
  --set cluster.leader.replicaCount=2 \
  --set cluster.worker.replicaCount=3

# Enable Kubernetes Gateway API support (install Gateway API CRDs separately)
helm install my-otoroshi otoroshi/otoroshi --set gatewayApi.enabled=true

# Cloud-provider LoadBalancer hints
helm install my-otoroshi otoroshi/otoroshi \
...
```sh
kubectl apply -f https://raw.githubusercontent.com/MAIF/otoroshi/master/kubernetes/helm/otoroshi/crds/crds.yaml
# or, via otoroshictl (includes any custom extension CRDs):
otoroshictl resources crds | kubectl apply -f -
```

See the [updating rbac and crds](#updating-rbac-and-crds-when-upgrading-otoroshi-using-otoroshictl) section below.
:::

For the full list of values (pod scheduling, security contexts, extraEnv, extra volumes, JAVA_OPTS, autoscaling, NetworkPolicy, …), see [`values.yaml`](https://github.com/MAIF/otoroshi/blob/master/kubernetes/helm/otoroshi/values.yaml).
...
:::
<Tabs>
<TabItem value="rbac.yaml" label="rbac.yaml" default>

```yaml include=../snippets/kubernetes/kustomize/base/rbac.yaml
```

</TabItem>
<TabItem value="crds.yaml" label="crds.yaml">

```yaml include=../snippets/kubernetes/kustomize/base/crds.yaml
```

</TabItem>
<TabItem value="redis.yaml" label="redis.yaml">

```yaml include=../snippets/kubernetes/kustomize/base/redis.yaml
```
...
```yaml include=../snippets/kubernetes/kustomize/overlays/simple/deployment.yaml
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple/dns.example
```
...
```yaml include=../snippets/kubernetes/kustomize/overlays/simple-baremetal/deployment.yaml
```

</TabItem>
<TabItem value="haproxy.example" label="haproxy.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal/haproxy.example
```

</TabItem>
<TabItem value="nginx.example" label="nginx.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal/nginx.example
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal/dns.example
```
...
```yaml include=../snippets/kubernetes/kustomize/overlays/simple-baremetal-daemonset/deployment.yaml
```

</TabItem>
<TabItem value="haproxy.example" label="haproxy.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal-daemonset/haproxy.example
```

</TabItem>
<TabItem value="nginx.example" label="nginx.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal-daemonset/nginx.example
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/simple-baremetal-daemonset/dns.example
```
...
```yaml include=../snippets/kubernetes/kustomize/overlays/cluster/deployment.yaml
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/cluster/dns.example
```
...
```yaml include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal/deployment.yaml
```

</TabItem>
<TabItem value="nginx.example" label="nginx.example">

```text include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal/nginx.example
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal/dns.example
```

</TabItem>
<TabItem value="dns.example2" label="dns.example">
...
```yaml include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal-daemonset/deployment.yaml
```

</TabItem>
<TabItem value="nginx.example" label="nginx.example">

```text include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal-daemonset/nginx.example
```

</TabItem>
<TabItem value="dns.example" label="dns.example">

```text include=../snippets/kubernetes/kustomize/overlays/cluster-baremetal-daemonset/dns.example
```
...
sudo kubectl get routes --all-namespaces
curl -X GET \
...
<Tabs>
<TabItem value="group.yaml" label="group.yaml" default>

```yaml include=../snippets/crds/group.yaml
```

</TabItem>
<TabItem value="apikey.yaml" label="apikey.yaml">

```yaml include=../snippets/crds/apikey.yaml
```

</TabItem>
<TabItem value="service-descriptor.yaml" label="service-descriptor.yaml">

```yaml include=../snippets/crds/service-descriptor.yaml
```

</TabItem>
<TabItem value="certificate.yaml" label="certificate.yaml">

```yaml include=../snippets/crds/certificate.yaml
```

</TabItem>
<TabItem value="jwt.yaml" label="jwt.yaml">

```yaml include=../snippets/crds/jwt.yaml
```

</TabItem>
<TabItem value="auth.yaml" label="auth.yaml">

```yaml include=../snippets/crds/auth.yaml
```

</TabItem>
<TabItem value="organization.yaml" label="organization.yaml">

```yaml include=../snippets/crds/organization.yaml
```

</TabItem>
<TabItem value="team.yaml" label="team.yaml">

```yaml include=../snippets/crds/team.yaml
```
...
```yaml include=../snippets/kubernetes/kustomize/base/coredns.yaml
```
...
In order to get CRD validation before manifest deployments right inside kubectl, you can deploy a validation webhook that will do the trick. Also check that you have `otoroshi.plugins.jobs.kubernetes.KubernetesAdmissionWebhookCRDValidator` request sink enabled (see the [enabling the admission webhook sinks](#enabling-the-admission-webhook-sinks) section below).

```yaml include=../snippets/kubernetes/kustomize/base/validation-webhook.yaml
```

### Enabling the admission webhook sinks

Both admission webhooks are served by Otoroshi *request sinks* — plugins that answer requests no route matched. The next-gen proxy engine, which is the default, only picks up request sinks declared in the global config under `plugins.config.ng`; the older `scripts.sinkRefs` list is read by the legacy engine only. Declaring them in both places covers either engine:
    ...
    "config": {
      "ng": [
        ...
        },
        {
          ...
          "config": {
            "plugin": "cp:otoroshi.plugins.jobs.kubernetes.KubernetesAdmissionWebhookCRDValidator"
          }
        },
        {
          "enabled": true,
          ...
          "plugin": "cp:otoroshi.next.plugins.wrappers.RequestSinkWrapper",
          "config": {
            "plugin": "cp:otoroshi.plugins.jobs.kubernetes.KubernetesAdmissionWebhookSidecarInjector"
          }
        }
      ]
    }
...
`RequestSinkWrapper` is what lets the next-gen engine run these plugins, which are written against the legacy request-sink API. They need no configuration of their own: the `KubernetesConfig` block used by the sinks falls back to `scripts.jobConfig.KubernetesConfig`, the very one the CRDs controller job uses.

:::warning
`plugins.config.ng` replaces Otoroshi's built-in plugin list rather than merging into it — a JSON deep merge does not merge arrays. Keep the entries that were already there, such as the default `NgClientCredentials` above.
:::

:::warning
When this configuration is passed through `OTOROSHI_INITIAL_CUSTOMIZATION`, it is only applied on the very first boot, while the datastore is still empty. On an upgrade — or against a pre-existing Redis — Otoroshi keeps the global config it already has, and the change must be applied by hand from the admin UI (Danger zone → Global plugins) or through the admin API.
:::

### Troubleshooting

`kubectl apply` failing with:

```
Internal error occurred: failed calling webhook "otoroshi-admission-webhook.otoroshi.io":
failed to call webhook: the server could not find the requested resource
```

means Otoroshi answered `404` on `/apis/webhooks/validation`, i.e. no request sink handled the call. The two usual causes are:

- the sinks are not registered for the running engine — see the section above;
- the `Host` the apiserver used does not match what the sinks accept. They only match `<otoroshiServiceName>.<namespace>.svc` and `<otoroshiServiceName>.<namespace>.svc.<clusterDomain>`, taken from `KubernetesConfig`. So `webhooks[].clientConfig.service.name` in the webhook manifest must be the very service named by `otoroshiServiceName` — in a Leader/Worker setup, that is the worker service. This is also the name the certificate Otoroshi generates for the endpoint is issued for, so a mismatch breaks the TLS handshake as well.

Note that the failure can stay invisible for a while: the manifests ship with `failurePolicy: Ignore`, and the CRDs controller job flips it to `Fail` on its first sync loop. Before the flip, the apiserver silently swallows the `404` and manifests are accepted unvalidated; after it, every apply fails.

## Easier integration with otoroshi-sidecar

Otoroshi can help you to easily use existing services without modifications while gettings all the perks of otoroshi like apikeys, mTLS, exchange protocol, etc. To do so, otoroshi will inject a sidecar container in the pod of your deployment that will handle call coming from otoroshi and going to otoroshi. To enable otoroshi-sidecar, you need to deploy the following admission webhook. Also check that you have `otoroshi.plugins.jobs.kubernetes.KubernetesAdmissionWebhookSidecarInjector` request sink enabled (see the [enabling the admission webhook sinks](#enabling-the-admission-webhook-sinks) section above).

```yaml include=../snippets/kubernetes/kustomize/base/sidecar-webhook.yaml
```
...
```yaml include=../snippets/kubernetes/kustomize/base/sidecar.yaml
```
...
<Image src="/img/docs/kubernetes-daikoku-integration-enabled.png" />

then when a user subscribe for an apikey, he will only see an integration token

<Image src="/img/docs/kubernetes-daikoku-integration-token.png" />

...
For user-facing authentication (login flows, SSO, session management), consider [authentication modules](./auth-modules) instead. For fine-grained token validation without a shared secret, look at [JWT verifiers](./jwt-verifiers.mdx). API keys and these other mechanisms are complementary and can be combined on the same route through the plugin chain.
...
- **[JWT Verifiers](./jwt-verifiers.mdx)** -- API keys can coexist with JWT verifiers on the same route. One of the extraction methods even allows embedding the API key credentials inside a signed JWT.
...
<Image src="/img/docs/models-apikey.png" />

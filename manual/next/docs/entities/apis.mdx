...
In a typical API gateway setup, each HTTP endpoint is configured as an individual [route](./routes.md). 
This works well for simple use cases, but as your platform grows, you end up with dozens or hundreds of routes that 
logically belong to the same service — they share the same backends, the same security policies, the same documentation. 
Managing them individually becomes tedious and error-prone.
...
In short, the API entity turns Otoroshi from a route-level proxy into a **full API Management platform**: 
you design, secure, publish, version, and document your APIs in one place.

<Image src="/img/docs/apis-home.png" />

## How it works at runtime

When Otoroshi starts or when an API is updated, each API is converted into a set of standard [NgRoute](./routes.md) objects that the proxy engine uses for request matching and forwarding. This conversion happens transparently:
...
2. Backend references are resolved — either from the API's local backends or from global [stored backends](./backends.md)
3. The plugin flow referenced by each route is attached as the route's plugin chain
4. Published [consumer plans](#plans) inject the appropriate security plugins (API key validation, JWT verification, mTLS checks, etc.) into each route automatically. Whatever the plan kind, the caller is turned into an [API key consumer](../topics/api-consumers.md) whose quotas and restrictions are enforced at the end of the access validation phase
5. If [testing mode](#testing) is enabled, additional draft routes are generated that require a special header to access
...
A [route](./routes.md) is a **low-level, flexible building block**. It gives you direct control over every aspect of HTTP proxying: exact domain matching, path patterns, headers, the full plugin chain, backend targets, TLS settings, etc. There are no constraints — you can wire anything to anything.
...
An API is a **higher-level, more opinionated entity** built on top of routes. It adds structure and guardrails: a shared domain, a context path, a lifecycle (`staging` → `published` → `deprecated` → `removed`), consumer plans, subscriptions, versioning, deployments, and documentation.
...
| `rateLimiting` | object | Rate limiting of the plan: `{ "strategy": { ... } }`. Every consumer of the plan is rate limited with that strategy. Defaults to the legacy strategy with the default quotas |
| `plugins` | object | Plugin chain applied for the consumers of this plan. See [plan plugins](#plan-plugins) |
| `validation` | object | How subscriptions are validated: `{ "kind": "auto" \| "manual" \| "webhook" \| "workflow" }` |
| `visibility` | object | Visibility of the plan in the developer portal: `{ "kind": "public" \| "semi_public" \| "private" \| "custom" }` |
...
Whatever the access mode, a published plan turns the caller into a **consumer**, and a consumer is
always an API key: the quotas, restrictions and rotation of the plan are enforced the same way for a
public plan and for a plan asking for client credentials. See
[API consumers and plugin flows](../topics/api-consumers.md) for the whole picture.

| Mode | Description | Plugins applied |
|------|-------------|-----------------|
| `keyless` | No credential required. The consumer is derived from an expression, the caller IP address by default | `NgExpressionApikeyExtractor` |
| `apikey` | Requires a valid API key. Configurable key patterns, restrictions, quotas, and rotation | `ApikeyCalls` |
| `jwt` | Requires a valid JWT token. The consumer is the client id claim of the token | `NgJwtApikeyExtractor` |
| `mtls` | Requires a TLS client certificate matching the configured subject/issuer patterns | `NgClientCertApikeyExtractor` |
...
| `oauth2-remote` | OAuth2 token verified against the OIDC settings of an auth. module. The consumer is the client id claim of the token | `NgOidcApikeyExtractor` |

Every published plan also adds `NgApiConsumerEnforcer` on the routes of the API. It runs last, requires
a consumer, and is the only place where a call is counted against the quotas of the plan.

All the access modes but `apikey` and `oauth2-local` generate an API key for a consumer they have
never seen, and they all share the `create_if_missing` flag for that. When it is on, the default,
the generated API key is persisted and shows up in the UI. When it is off, the identity only lives
for the duration of the call. In both cases the caller is served and its consumption is counted.

### Keyless access mode configuration

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `expr` | string | `${req.ip}` | Expression building the consumer identity of a public call |
| `create_if_missing` | boolean | `true` | Persist the generated API key |

:::warning
A missing value resolves to `no-header-<field>` and puts every such caller on one shared quota. Use
an empty default (`${req.headers.x-consumer:}`) to have those calls rejected instead.
:::

### JWT access mode configuration

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `verifier` | string | | ID of the [JWT verifier](./jwt-verifiers.md) validating the token |
| `client_id_path` | string | `client_id` | Claim holding the client id of the consumer. A JSON path such as `$.azp` works too |
| `create_if_missing` | boolean | `true` | Persist the generated API key |

### mTLS access mode configuration

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `regex_subject_dns` | array of string | `[]` | Regex patterns the subject DN of the client certificate may match |
| `regex_issuer_dns` | array of string | `[]` | Regex patterns the issuer DN may match. A certificate matching either list is accepted. Both lists empty means any trusted certificate is accepted |
| `client_id_field` | string | `null` | DN attribute holding the client id (`UID`, `CN`, ...). Left empty, the identity is derived from the whole subject and the serial number. A certificate missing the named attribute is rejected |
| `create_if_missing` | boolean | `true` | Persist the generated API key |

### OAuth2 remote access mode configuration

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `verifier` | string | | ID of the [auth. module](./auth-modules.md) whose OIDC settings validate the token |
| `client_id_path` | string | `client_id` | Claim holding the client id of the consumer (`client_id`, `azp` or `cid` depending on the vendor) |
| `fetch_user` | boolean | `false` | Call the userinfo endpoint of the module and store the profile of the token holder in a metadata of the API key. The call stays a consumer call, no user session is attached to it |
| `user_metadata_key` | string | `user_profile` | Metadata key holding that profile |
| `create_if_missing` | boolean | `true` | Persist the generated API key |
...
The `rateLimiting` property of a plan holds the throttling strategy handed over to every consumer of
the plan, whether it got its API key through a subscription or was generated on the fly by the access
mode. A plan without an explicit strategy falls back on the legacy one with the default quotas, which
is what an API key created by hand gets.
...
```json
{
  "rateLimiting": {
    "strategy": {
      ...
      "quota": { "window": 100, "daily": 10000, "monthly": 300000 }
    }
  }
}
```
...
A plan can bring its own plugins. They are applied on top of the plugin chain of the route, for the
consumers of that plan only. See
...
| `flow_ref` | string | `null` | ID of one of the [flows](#flows) of the API to use instead of listing the plugins here. An inline chain extends the referenced flow, it does not replace it |
    ...
    "plugins": [
      {
        ...
        "exclude": [],
        "config": {
          "headers": { "X-Plan": "${apikey.api.plan}" }
        }
      }
    ]
  }
...
| `subscription_kind` | string | Kind of subscription: `apikey`, `mtls`, `keyless`, `oauth2-local`, `oauth2-remote`, `jwt`. It has to be the access mode of the plan |
...
A subscription is refused with a `400` when its `subscription_kind` is not the access mode of its plan.
A subscription to an `apikey` or an `oauth2-local` plan is given an API key built from the plan, and
referenced in `token_refs` as `{"apikey": "<client id>"}`: the consumers of these plans call with an
API key, and a subscription is the only way to get one. The other access modes build their consumer at call time, their subscriptions
...
* [API Keys](./apikeys.mdx) - Credentials generated when subscribing with `apikey` access mode
...
* [API consumers and plugin flows](../topics/api-consumers.md) - How a plan identifies its callers and how plan plugins are applied

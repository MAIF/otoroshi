...
sidebar_label: Service descriptors (removed)
sidebar_position: 17
---

import { Image } from '@site/src/components/Image';
import { NewTabLink } from '@site/src/components/NewTabLink';

# Service descriptors

:::danger[Removed in Otoroshi v18.0.0]
Service Descriptors **no longer exist**. The entity, its admin API, its pages in the admin UI and its Kubernetes CRD have all been removed in `v18.0.0`. [Routes](./routes.md) are now the only way to configure HTTP proxying, and your existing descriptors are migrated to Routes automatically the first time a `v18` instance starts.

**Read [Sunsetting Service Descriptors](../topics/deprecating-sd.mdx) instead.** That page covers the automatic migration, what keeps working, what has been removed and how to convert a descriptor you still hold as JSON or YAML.

This page is kept as a **historical reference only**: nothing described below can be configured anymore. It is still useful when you need to make sense of a `v17` export, of a `service-descriptors-backup.json` file or of an old `ServiceDescriptor` manifest — the [Routes](./routes.md) page documents how to do the same things today.
:::

## Context and removal

Service Descriptors were the **original and historical way** to configure HTTP proxying in Otoroshi. From the very first version of the project, they served as the single entity used to define how incoming requests should be matched, authenticated, transformed, and forwarded to backend services.

### Why they were removed

Service Descriptors followed a **monolithic design**: frontend matching (domains, paths), backend targets, authentication, API key constraints, CORS, header manipulation, JWT verification, plugins, and many other concerns were all configured within a single large entity. While this approach was simple for basic use cases, it became increasingly difficult to manage, reuse, and compose as configurations grew more complex.
...
- **[Routes](./routes.md)** provide fine-grained, low-level HTTP routing with a clean separation between frontend matching, backend targeting, and plugin chains. Routes are more flexible, more composable, and support the full plugin system with proper ordering and scoping.
- **[APIs](./apis.mdx)** group multiple routes together for full API management, providing a higher-level abstraction when you need to manage an entire API surface as a single unit.
...
They were deprecated for more than 4 years. `v17.0.0` shipped the migration tooling and `v18.0.0` removed the entity itself.

Nothing changed for your traffic along the way: the proxy engine had been converting Service Descriptors into Routes at runtime since `v1.5.3`, so what disappeared in `v18.0.0` is the entity you used to configure, not the way requests are proxied.

- **Existing descriptors are migrated for you.** The first time a `v18` instance starts on a datastore that still contains Service Descriptors, a migration job writes a backup to `./service-descriptors-backup.json` and converts each descriptor into a Route, **keeping the same id**. There is nothing to enable and nothing to run.
- **Old exports still import.** `POST /api/otoroshi.json` and `POST /api/import` still read the `serviceDescriptors` key of a `v17` backup and convert each entry to a Route on the fly.
- **Descriptors you hold outside of the datastore can still be converted.** `POST /api/routes/_from_service_descriptor` turns Service Descriptor JSON into Route JSON without storing anything, and `POST /api/new/resources` turns a `proxy.otoroshi.io/ServiceDescriptor` manifest into a `proxy.otoroshi.io/Route` one.

All of this — including what has been removed from the admin API, the admin UI and the Kubernetes integration — is detailed in the [Sunsetting Service Descriptors](../topics/deprecating-sd.mdx) topic.

---

:::note
Everything below describes the Service Descriptor entity and its admin UI form **as they were in `v17`**, and is kept for reference. The pages, buttons and settings it mentions no longer exist in the admin UI.
:::

## Overview

Services, or service descriptors, let you declare how to proxy a call from a domain name to another domain name (or multiple domain names).

<Image src="/img/docs/models-service.png" />
...
* <NewTabLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Credentials">Access-Control-Allow-Credentials</NewTabLink>
* <NewTabLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Origin">Access-Control-Allow-Origin</NewTabLink>
* <NewTabLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Max-Age">Access-Control-Max-Age</NewTabLink>
* <NewTabLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Methods">Access-Control-Allow-Methods</NewTabLink>
* <NewTabLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Headers">Access-Control-Allow-Headers</NewTabLink>

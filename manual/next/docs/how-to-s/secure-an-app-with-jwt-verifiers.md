...
Let's start by validating all incoming request tokens tokens on our simple route created in the [Before you start](#before-you-start) section.
...
A simple way to generate a token is to use [jwt.io](http://jwt.io). Once navigate, define `HS512` as `alg` in header section and insert `otoroshi` as verify signature secret. 

...
1. Navigate to [http://otoroshi.oto.tools:8080/bo/dashboard/routes](http://otoroshi.oto.tools:8080/bo/dashboard/routes) and click on the `create new route` button
...
Navigate to [http://myservice.oto.tools:8080/api/test](http://myservice.oto.tools:8080/api/test) again. If the service is configured, you should have a `Service Not found error`.

The expected error on the `/api/test`, indicate that an api key is required to access to this part of the backend service.

Navigate to any other routes which are not starting by `/api/*` like [http://myservice.oto.tools:8080/test/bar](http://myservice.oto.tools:8080/test/bar)


### Generate an api key to request secure services

Navigate to [http://otoroshi.oto.tools:8080/bo/dashboard/apikeys/add](http://otoroshi.oto.tools:8080/bo/dashboard/apikeys/add) or when clicking on the **Add apikey** button on the sidebar.

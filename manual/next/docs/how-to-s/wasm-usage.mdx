...
- otoroshi `wasmo`: a code editor in the browser that let you write your plugin in `Rust`, `TinyGo`, `Javascript` or `Assembly Script` without having to think about compiling it to WASM (you can find a complete tutorial about it [here](../how-to-s/wasmo-installation.mdx))
...
If you need to expose more than a HTML page, we highly recommend to use the [Zip Backend plugin](../how-to-s/zip-backend-plugin.mdx)

...
<Image src="/img/docs/push-to-elastic.png" />
...
<Image src="/img/docs/service-healthcheck.png" />
...
<Image src="/img/docs/service-live-stats.png" />

### Service analytics

You can also get some aggregated metrics. The analytics page can be accessed from the sidebar of the service page.

<Image src="/img/docs/service-analytics.png" />

## New proxy engine

### Debug reporting

when using the [new proxy engine](./engine.mdx), when a route or the global config. enables traffic capture using the `debug_flow` flag, events of type `RequestFlowReport` are generated

### Traffic capture

when using the [new proxy engine](./engine.mdx), when a route or the global config. enables traffic capture using the `capture` flag, events of type `TrafficCaptureEvent` are generated. It contains everything that compose otoroshi input http request and output http responses

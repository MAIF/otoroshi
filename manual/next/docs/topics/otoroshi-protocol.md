...
Scala
:   > **Included file**: `../snippets/filter.scala`

Java
:   > **Included file**: `../snippets/filter.java`


## Using `otoroshictl` to bootstrap a proxy implementing the Otoroshi protocol

While the protocol can be implemented directly inside your service, [**`otoroshictl`**](https://cloud-apim.github.io/otoroshictl/) from our friends at [Cloud AIM](https://www.cloud-apim.com/) provides a convenient way to bootstrap a proxy that implements the protocol on behalf of your application.

This is particularly useful when:

* your service cannot be modified,
* you want to secure local processes,
* you need a sidecar or tunnel-based architecture,
* you are exposing services through Otoroshi tunnels or remote proxies.

### What the proxy does

A proxy created with `otoroshictl challenge proxy` acts as an intermediary that:

1. Receives requests forwarded by Otoroshi.
2. Handles the **challenge protocol**:

   * reads the `Otoroshi-State` header,
   * validates the token (V1 or V2),
   * returns the expected response token in `Otoroshi-State-Resp`.
3. Validates and exposes the **info token** (`Otoroshi-Claim`) when enabled.
4. Forwards the request to your local or remote service.

This allows your service to remain unaware of the protocol while still benefiting from secure communication.

### Info token support

When the **info token** feature is enabled on the route, Otoroshi sends a JWT in the `Otoroshi-Claim` header containing contextual information about the request and client.

The proxy generated by `otoroshictl` can:

* validate the signature,
* expose claims to the downstream service,
* or forward the token unchanged for application-level verification.

The info token is configurable and signed using the shared key or custom algorithm defined in the route configuration. ([maif.github.io][2])

### Creating a proxy with `otoroshictl`

Example:

```bash
otoroshictl challenge proxy \
  --port 9000 \
...
* starts a local proxy listening on `127.0.0.1:9000`
* implements the Otoroshi protocol challenge with HS512 validation (with secret `my-secret`)
* forwards traffic to `http://localhost:8080`.

You can then configure your Otoroshi route to target this proxy instead of your service directly.

### Typical architecture

```
Client → Otoroshi → otoroshictl proxy → your service
```

Benefits:

* zero modification of the service,
* protocol compliance out of the box,
* improved security posture,
* ideal for local development and remote tunnels.

### Full documentation

You can configure a lot of stuff with the `otoroshictl challenge command` like the header names used for JWT token, the algorithms used (input/output), validation, asymmetric keys, etc. You can pass the options as command line options or environment variables.

You can find the full documentation of the `otoroshictl challenge proxy` command [here](https://cloud-apim.github.io/otoroshictl/docs/tools/proxy)

...
sequenceDiagram
    ...
    C->>O: Otoroshi-Client-Id / Otoroshi-Client-Secret
    O->>B: Otoroshi-State: yyy
    Note over O,B: + Otoroshi-Request-Id<br/>+ Otoroshi-Proxied-Host<br/>+ Otoroshi-Claim (JWT)
    B->>O: Otoroshi-State-Resp: yyy
    O->>C: Otoroshi-Proxy-Latency: 42
    Note over O,C: + Otoroshi-Upstream-Latency<br/>+ Daily-Calls-Remaining<br/>+ Monthly-Calls-Remaining
```
...
sequenceDiagram
    ...
    C->>O: Otoroshi-Client-Id / Otoroshi-Client-Secret
    O->>B: Otoroshi-State: jwt1 (signed JWT with state claim)
    Note over O,B: + Otoroshi-Request-Id<br/>+ Otoroshi-Proxied-Host<br/>+ Otoroshi-Claim (JWT)
    B->>O: Otoroshi-State-Resp: jwt2 (JWT with state-resp claim, aud=Otoroshi)
    O->>C: Otoroshi-Proxy-Latency: 42
    Note over O,C: + Otoroshi-Upstream-Latency<br/>+ Daily-Calls-Remaining<br/>+ Monthly-Calls-Remaining
```
...
<Image src="/img/docs/sec-com-signing-bis.png" />

using another signing algo.

<Image src="/img/docs/sec-com-signing-2-bis.png" />

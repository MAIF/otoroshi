...
const expressions = [
  ...
  },
  {
    ...
    editor: [
      ...
      { expression: "${req.ip_safe}", description: "the client ip address that can be trusted for security decisions: the connection address when `trustXForwarded` is disabled, otherwise resolved through the trusted proxies when some are configured, or the leftmost `X-Forwarded-For` entry when none is" },
      { expression: "${req.ip_from_socket}", description: "the address of the peer that opened the connection, without any header involved" },
      { expression: "${req.ip_from_trusted_proxy}", description: "the client ip address resolved by walking the `Forwarded` or `X-Forwarded-For` chain, only when `trustXForwarded` is enabled and the connection comes from a trusted proxy. falls back to the connection address" },

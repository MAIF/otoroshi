...
const expressions = [
  ...
  },
  {
    ...
    editor: [
      ...
      { expression: "${req.ip}", description: "the client ip address, the same as `${req.ip_safe}` unless `useLegacyClientIpAddress` is enabled, in which case it is the leftmost `X-Forwarded-For` entry when `trustXForwarded` is enabled and the connection address otherwise" },
      { expression: "${req.ip_address}", description: "same as `${req.ip}`" },
      { expression: "${req.ip_safe}", description: "the client ip address resolved according to the forwarded headers settings: the connection address when `trustXForwarded` is disabled, otherwise resolved through the trusted proxies when some are configured, or the first valid ip address of the client address header (`X-Forwarded-For` by default) when none is, falling back to the connection address when none is available" },
      { expression: "${req.ip_from_socket}", description: "the address of the peer that opened the connection, without any header involved" },
      { expression: "${req.ip_from_trusted_proxy}", description: "the client ip address resolved by walking the chain of the client address header (`X-Forwarded-For` by default, `Forwarded`, `X-Real-IP`, ...), only when `trustXForwarded` is enabled and the connection comes from a trusted proxy. falls back to the connection address" },
      { expression: "${req.ip_from_xff}", description: "the leftmost address of the `X-Forwarded-For` header, whatever sent it. falls back to the connection address" },
      { expression: "${req.ip_from_xforwarded_header}", description: "same as `${req.ip_from_xff}`" },
    ...
    values: [
      ...
      { expression: "${req.ip}", value: "10.0.0.4" },
      { expression: "${req.ip_safe}", value: "10.0.0.4" },
      { expression: "${req.ip_from_socket}", value: "192.168.1.12" },
      { expression: "${req.ip_from_trusted_proxy}", value: "10.0.0.4" },
      { expression: "${req.ip_from_xff}", value: "10.0.0.4" },

...
const expressions = [
  ...
  },
  {
    ...
    editor: [
      ...
      { expression: "${req.ip_safe}", description: "the client ip address that can be trusted for security decisions: the connection address when `trustXForwarded` is disabled, otherwise resolved through the trusted proxies when some are configured, or the leftmost entry of the client address header (`X-Forwarded-For` by default) when none is" },

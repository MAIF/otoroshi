...
Let's Encrypt must be enabled in the @ref:[Global Config](./global-config.md) with a valid ACME server URL.
...
* @ref:[Routes](./routes.md) - Routes use certificates for TLS termination
* @ref:[Backends](./backends.md) - Backend targets can use client certificates for mTLS
* @ref:[Auth Modules](./auth-modules.md) - OIDC/OAuth modules can use certificates for provider communication
* @ref:[JWT Verifiers](./jwt-verifiers.md) - Key pairs can be used for JWT signing and verification

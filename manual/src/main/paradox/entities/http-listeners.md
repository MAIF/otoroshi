...
Routes can be bound to specific listeners using the `bound_listeners` field on the @ref:[route](./routes.md) entity. When a listener is configured as `exclusive`, only routes that explicitly reference this listener in their `bound_listeners` array will be served on its port.

...
| `route` | object | A full @ref:[route](./routes.md) definition used as the template content |
...
Otoroshi can have a global default route template configured in the @ref:[Global Config](./global-config.md) (under `templates.routeTemplate`). When set, all new routes will be initialized from this template unless another template is explicitly selected.

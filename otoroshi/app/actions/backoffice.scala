          ...
          env.datastores.backOfficeUserDataStore.findById(id).flatMap {
            case Some(user) => {
              env.datastores.backOfficeUserDataStore.blacklisted(user.email).flatMap {
                ...
                case false => checker.check(req, user) {
...
class AdminClearanceChecker()(implicit env: Env) {
  ...
  def checkAdminApiCall(req: Request[_], user: BackOfficeUser, groupId: String)(f: => Future[Result]): Future[Result] = {
    if (!req.path.startsWith("/bo/api/proxy/")) {
      ...
      case "/api/groups" if req.method == "POST" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
      ...
      case "/api/globalconfig" if req.method == "PATCH" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
      case "/api/otoroshi.json" if req.method == "GET" => FastFuture.successful(Results.Ok(Json.obj()))
      case "/api/otoroshi.json" if req.method == "POST" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
    ...
    user.authorizedGroup match {
      ...
      case Some(groupId) => {
        ...
        req.path match {
          ...
          case "/bo/api/sessions" if req.method == "DELETE" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
          case r"/bo/api/sessions/[^/]+" if req.method == "DELETE" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
          case "/bo/api/apps/sessions" if req.method == "GET" => FastFuture.successful(Results.Ok(Json.arr()))
          case "/bo/api/apps/sessions" if req.method == "DELETE" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
          case r"/bo/api/apps/sessions/[^/]+" if req.method == "DELETE" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
          ...
          case "/bo/api/services/map" => FastFuture.successful(Results.Ok(Json.obj("name" -> "Otoroshi Services", "children" -> Json.arr())))
          case r"/bo/api/loggers" if req.method == "GET" => f
          case r"/bo/api/loggers/[^/]+/level" if req.method == "GET" => f
          case r"/bo/api/loggers/$name@([^/]+)/level" if req.method == "PUT" => FastFuture.successful(Results. Ok(Json.obj("name" -> name, "oldLevel" -> "--", "newLevel" -> "--")))
          case r"/bo/api/services/[^/]+/circuitbreakers" if req.method == "DELETE" => FastFuture.successful(Results.Ok(Json.obj("done" -> false)))
          case "/bo/api/search/services" => f.flatMap(r => filterBody(r, groupId, "groupId"))
          case "/" => f
          case "/robot.txt" => f
          case "/error" => f
          case "/auth0error" => f
          case "/bo/u2f/register/start" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          case "/bo/u2f/register/finish" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          case "/bo/u2f/login/start" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          case "/bo/u2f/login/finish" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          case "/bo/u2f/admins" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          case r"/bo/u2f/admins/[^/]+/[^/]+" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))
          ...
          case r"/bo/simple/admins/[^/]+" => FastFuture.successful(Results.Unauthorized(Json.obj("error" -> "unauthorized")))

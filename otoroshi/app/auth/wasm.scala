...
case class WasmAuthModuleConfig(
  ...
  override def save()(implicit ec: ExecutionContext, env: Env): Future[Boolean] = env.datastores.authConfigsDataStore.set(this)
...
class WasmAuthModule(val authConfig: WasmAuthModuleConfig) extends AuthModule {
    ...
    authConfig.wasmRef.flatMap(env.proxyState.wasmPlugin).map { plugin =>
      ...
      WasmUtils.execute(plugin.config, "pa_login_page", input, None, None).map {
        ...
        case Right(output) => {
          ...
          Results.Status(response.select("status").asOpt[Int].getOrElse(200))
            .apply(body)
      ...
      Results.InternalServerError(Json.obj("error" -> "internal_server_error", "error_description" -> "wasm module not found")).vfuture
    }
  }
  ...
  override def paLogout(request: RequestHeader, user: Option[PrivateAppsUser], config: GlobalConfig, descriptor: ServiceDescriptor)(implicit ec: ExecutionContext, env: Env): Future[Either[Result, Option[String]]] = {
    authConfig.wasmRef.flatMap(env.proxyState.wasmPlugin).map { plugin =>
      ...
      Results.InternalServerError(Json.obj("error" -> "internal_server_error", "error_description" -> "wasm module not found")).left.vfuture
    }
  }

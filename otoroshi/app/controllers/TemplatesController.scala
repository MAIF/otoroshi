...
class TemplatesController(ApiAction: ApiAction, cc: ControllerComponents)(implicit env: Env)
  ...
  def process(json: JsValue, req: RequestHeader): JsValue = {
    val over = req.queryString.filterNot(_._1 == "rawPassword").map(t => Json.obj(t._1 -> t._2.head)).foldLeft(Json.obj())(_ ++ _)
  ...
  def initiateAuthModule() = ApiAction { ctx =>
    ctx.request.getQueryString("mod-type") match {
      case Some("oauth2") => Ok(process(GenericOauth2ModuleConfig(
        id = IdGenerator.token,
        name = "New auth. module",
        desc = "New auth. module"
      ).asJson, ctx.request))
      case Some("oauth2-global") => Ok(process(GenericOauth2ModuleConfig(
        id = IdGenerator.token,
        name = "New auth. module",
        desc = "New auth. module"
      ).asJson, ctx.request))
      case Some("basic") => Ok(process(BasicAuthModuleConfig(
        id = IdGenerator.token,
        name = "New auth. module",
        desc = "New auth. module"
      ).asJson, ctx.request))
      case Some("ldap") => Ok(process(LdapAuthModuleConfig(
        id = IdGenerator.token,
        name = "New auth. module",
        desc = "New auth. module",
      ...
      case _ => Ok(process(BasicAuthModuleConfig(
        id = IdGenerator.token,
        name = "New auth. module",
        desc = "New auth. module"
      ).asJson, ctx.request))
  ...
  def initiateSimpleAdmin() = ApiAction { ctx =>
    val pswd: String = ctx.request.getQueryString("rawPassword").map(v => BCrypt.hashpw(v, BCrypt.gensalt())).getOrElse(BCrypt.hashpw("password", BCrypt.gensalt()))
    Ok(process(Json.obj(

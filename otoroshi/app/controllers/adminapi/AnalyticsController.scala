        ...
        env.datastores.serviceDescriptorDataStore.findById(serviceId).flatMap {
          ...
          case Some(desc) => {
            ...
            for {
              ...
              favgduration         =
                analyticsService.fetchAvgDuration(Some(ServiceDescriptorFilterable(desc)), fromDate, toDate)
              favgoverhead         =
                analyticsService.fetchAvgOverhead(Some(ServiceDescriptorFilterable(desc)), fromDate, toDate)
          ...
          futureFilterable.flatMap {
            ...
            case Some(filterable) => {
              ...
              val parts = Seq(
                ...
                Part(
                  "dataInStats",
                  () => analyticsService.fetchDataInStatsHistogram(Some(filterable), fromDate, toDate)
                ),
                Part(
                  "dataOutStats",
      ...
      env.datastores.globalConfigDataStore.singleton().flatMap { globalConfig =>
        ...
        env.datastores.serviceDescriptorDataStore
          ...
          .flatMap {
            ...
            case Some(desc)                           =>
              analyticsService.fetchServicesStatus(Seq(desc), fromDate, toDate).map {
                case Some(value) => Ok(value)
                case None        => NotFound(Json.obj("error" -> "No entity found"))
              }
          }
      ...
      env.datastores.globalConfigDataStore.singleton().flatMap { globalConfig =>
        ...
        env.datastores.serviceDescriptorDataStore
          ...
          .flatMap {
            ...
            case Some(desc) if !ctx.canUserRead(desc) => ctx.fforbidden
            case Some(desc)                           =>
              analyticsService.fetchServiceResponseTime(desc, fromDate, toDate).map {
                case Some(value) =>
                  logger.warn("ok")
                  Ok(value)
                case None        => NotFound(Json.obj("error" -> "No entity found"))

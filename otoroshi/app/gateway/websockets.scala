                                          ...
                                          env.circuitBeakersHolder
                                            ...
                                            case AllCircuitBreakersOpenException =>
                                              Errors
                                                ...
                                                .asLeft[WSFlow]
                                            case error
                                                if error.getMessage.toLowerCase().contains("connection refused") =>
                                              Errors
                                                .craftResponseResult(
                                              ...
                                              env.datastores.apiKeyDataStore
                                                ...
                                                .flatMap {
                                                  case Some(apiKey) => {
                                                    ...
                                                    val httpPath =
                                                      Option(jwt.getClaim("httpPath"))
                                                        .filterNot(_.isNull)
                                                        .map(_.asString())
                                                    val httpVerb =
                                                      Option(jwt.getClaim("httpVerb"))
                                                        .filterNot(_.isNull)
                                                        .map(_.asString())
                                                    val httpHost =
                                                      Option(jwt.getClaim("httpHost"))
                                                        .filterNot(_.isNull)
                                                        .map(_.asString())
                                                    val verifier =
                                  ...
                                  globalConfig.withinThrottlingQuota().map(within => (globalConfig, within)) flatMap {
                                    tuple =>
                                      ...
                                      env.datastores.globalConfigDataStore
                                        .incrementCallsForIpAddressWithTTL(from)
                                        .flatMap { secCalls =>
                                          env.datastores.globalConfigDataStore.quotaForIpAddress(from).map {
                                            maybeQuota =>
                                              (secCalls, maybeQuota)
                                          }
                                        } flatMap { r =>

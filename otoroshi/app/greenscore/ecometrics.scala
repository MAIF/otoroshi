...
class GlobalScore {
  ...
  def updateRoute(routeId: String, overhead: Long,
    ...
    routesScore.getOrElseUpdate(routeId, new RouteScore())
      .update(overhead, overheadWithoutCircuitBreaker, circuitBreakerDuration, duration, plugins, backendId, dataIn, dataOut, headers, headersOut)
  }
...
class EcoMetrics(env: Env)  {
  ...
  def updateRoute(routeId: String,
                  ...
                  headers: Long,
                  headersOut: Long) = {
    registry.updateRoute(routeId, overhead, overheadWoCb, cbDuration, duration, plugins, backendId, dataIn, dataOut, headers, headersOut)
  }
}
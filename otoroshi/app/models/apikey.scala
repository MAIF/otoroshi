  ...
  implicit val fmt   = new Format[RemainingQuotas] {
    ...
    override def reads(json: JsValue): JsResult[RemainingQuotas] = Try {
      RemainingQuotas(
        ...
        allowed = json.select("allowed").asOptBoolean.getOrElse(false)
      )
    } match {
...
case class ApiKey(
  ...
  def withinQuotasAndRotationQuotas(allowed: Boolean = false)(implicit
      ec: ExecutionContext,
      env: Env
    ...
    for {
      ...
      val within = (allowed || quotas.remainingCallsPerWindow > 0) &&
...
object ApiKeyHelper {
  ...
  def passWithApiKeyFromCache(
    ...
    def tooMuchRequests(apikey: ApiKey, quotas: RemainingQuotas) = {
      attrs.put(otoroshi.plugins.Keys.ErrorApiKeyKey -> apikey)
      sendQuotasExceededError(apikey, quotas)
      error(
    ...
    detectApikeyTuple(req, constraints, attrs) match {
      ...
      case Some(apikeyTuple) =>
        validateApikeyTuple(req, apikeyTuple, constraints, service, attrs) match {
          ...
          case Right(apikey)
              if apikey.restrictions.handleRestrictions(service, None, route, Some(apikey), req, attrs)._1 => {
            apikey.restrictions

  ...
  private val specs: Seq[Spec] = Seq(
    Spec(
      ...
      Seq(
        ...
        w("errors-total", "Total errors", "errors_total", "metric", 1, 1, options = Json.obj("format" -> "count")),
        w("rps", "Requests per second", "requests_per_second", "line", 4, 2, options = Json.obj("format" -> "rps")),
        w(
          "avg-duration",
    ...
    Spec(
      ...
      Seq(
        w(
          "latency-p",
          "Latency percentiles",
          "duration_percentiles_ts",
          "line",
          4,
          2,
          options = Json.obj("format" -> "ms")
        ),
        w("avg-overhead", "Average overhead", "overhead_avg_ts", "line", 2, 2, options = Json.obj("format" -> "ms")),
        w(
          "avg-duration",
          "Average response time",
          "duration_avg_ts",
    ...
    Spec(
      ...
      Seq(
        w("err-rate", "Error rate over time", "error_rate_ts", "line", 4, 2, options = Json.obj("format" -> "percent")),
        w("err-total", "Total errors", "errors_total", "metric", 1, 1),
        w("top-err-routes", "Top error routes", "top_error_routes", "bar", 3, 2, params = Json.obj("top_n" -> 10)),
        w(
          "status-classes",
          "Status classes over time",
          "requests_by_status_class_ts",
    ...
    Spec(
      ...
      Seq(
        w("top-routes", "Top routes", "requests_by_route", "bar", 2, 2, params = Json.obj("top_n" -> 10)),
        w("top-apis", "Top APIs", "requests_by_api", "bar", 2, 2, params = Json.obj("top_n" -> 10)),
        w("top-domains", "Top domains", "requests_by_domain", "bar", 2, 2, params = Json.obj("top_n" -> 10)),
        w(
          "traffic-io",
          "Data in / out over time",
          "traffic_in_out_ts",
          "area",
          2,
          2,
    ...
    Spec(
      ...
      Seq(
        w("top-apikeys", "Top API keys", "requests_by_apikey", "bar", 2, 2, params = Json.obj("top_n" -> 10)),
        w("top-users", "Top users", "requests_by_user", "bar", 2, 2, params = Json.obj("top_n" -> 10)),
        w("top-countries", "Top countries", "requests_by_country", "bar", 2, 2, params = Json.obj("top_n" -> 10)),

...
case class NgHttpClientCacheConfig(maxAgeSeconds: Long, methods: Seq[String], status: Seq[Int], mimeTypes: Seq[String]) extends NgPluginConfig {
  ...
  val format = new Format[NgHttpClientCacheConfig] {
    override def reads(json: JsValue): JsResult[NgHttpClientCacheConfig] = Try {
      NgHttpClientCacheConfig(
        ...
        status = json.select("status").asOpt[Seq[Int]]
...
class NgHttpClientCache extends NgRequestTransformer {
  ...
  private def methodMatch(ctx: NgTransformerResponseContext, config: NgHttpClientCacheConfig)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Boolean = {
    config.methods.map(_.toLowerCase().trim).contains(ctx.request.method.toLowerCase())
  }

  private def statusMatch(ctx: NgTransformerResponseContext, config: NgHttpClientCacheConfig)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Boolean = {
    config.status.contains(ctx.otoroshiResponse.status)
  }

  private def contentMatch(ctx: NgTransformerResponseContext, config: NgHttpClientCacheConfig)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Boolean = {
    ctx.otoroshiResponse.contentType match {
      case None if config.mimeTypes.contains("*") => true
      case None => false
      case Some(contentType) => config.mimeTypes.exists(mt => contentType.startsWith(mt) || RegexPool.apply(mt).matches(contentType))
    }
  }
  ...
  override def transformResponse(ctx: NgTransformerResponseContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpResponse]] = {
    val config = ctx.cachedConfig(internalName)(NgHttpClientCacheConfig.format).getOrElse(NgHttpClientCacheConfig.default)
    if (methodMatch(ctx, config) && statusMatch(ctx, config) && contentMatch(ctx, config)) {
      ...
      val expires = now.plusSeconds(config.maxAgeSeconds.toInt).toString("EEE, dd MMM yyyy HH:mm:ss 'GMT'", Locale.ENGLISH)
      ctx.otoroshiResponse.copy(headers = ctx.otoroshiResponse.headers ++ Map(

...
object AdditionalCookieOutConfig {
  ...
  def configSchema: Option[JsObject] = Some(Json.obj(
    ...
    "sameSite" -> Json.obj("label" -> "Same Site", "type" -> "select", "props" -> Json.obj(
        "isClearable" -> true,
        "options" -> Json.arr(
...
class AdditionalCookieIn extends NgRequestTransformer {
  ...
  override def transformRequest(ctx: NgTransformerRequestContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
    val config = ctx.cachedConfig(internalName)(AdditionalCookieInConfig.format.reads).getOrElse(AdditionalCookieInConfig.default)
...
class AdditionalCookieOut extends NgRequestTransformer {
  ...
  override def transformResponse(ctx: NgTransformerResponseContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpResponse]] = {
    val config = ctx.cachedConfig(internalName)(AdditionalCookieOutConfig.format.reads).getOrElse(AdditionalCookieOutConfig.default)
...
class RemoveCookiesIn extends NgRequestTransformer {
  ...
  override def transformRequest(ctx: NgTransformerRequestContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
    val config: RemoveCookiesInConfig = ctx.cachedConfig(internalName)(RemoveCookiesInConfig.format.reads).getOrElse(RemoveCookiesInConfig.default)
    Right(ctx.otoroshiRequest.copy(
...
class CookiesValidation extends NgAccessValidator {
  ...
  override def access(ctx: NgAccessContext)(implicit env: Env, ec: ExecutionContext): Future[NgAccess] = {
    val config = ctx.cachedConfig(internalName)(CookiesValidationConfig.format.reads).getOrElse(CookiesValidationConfig())

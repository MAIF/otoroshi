...
case class Http3SwitchConfig(ma: Int = 3600, domain: Option[String] = None, protocols: Seq[String] = Seq.empty) extends NgPluginConfig {
...
class Http3Switch extends NgRequestTransformer {
  ...
  override def description: Option[String]                 = "This plugin injects additional alt-svc header to switch to the http3 server".some
  ...
  override def transformResponseSync(
    ...
    val protocols: Seq[String] = if (config.protocols.isEmpty) Http3.supportedApplicationProtocols().toSeq else config.protocols
    val domain: String = config.domain.getOrElse("")
    val port: Int = ReactorNettyServerConfig.parseFromWithCache(env).http3.exposedPort
    val headers: Map[String, String] = Map("alt-svc" -> protocols.map(p => s"""$p=\"$domain:$port\"; ma=${config.ma}""").reverse.mkString(", "))

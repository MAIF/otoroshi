...
class UserLogoutEndpoint extends NgBackendCall {
  ...
  override def multiInstance: Boolean                      = true
  override def core: Boolean                               = true
  override def name: String                                = "User logout endpoint"
  override def description: Option[String]                 = "This plugin logout the current user".some
  override def defaultConfigObject: Option[NgPluginConfig] = None
  override def useDelegates: Boolean                       = false
  override def noJsForm: Boolean                           = true
  ...
  override def callBackend(
    ...
    ctx.user match {
      ...
      case Some(_) => {
        GatewayRequestHandler.removePrivateAppsCookies(ctx.route, ctx.rawRequest, ctx.attrs).map { result =>

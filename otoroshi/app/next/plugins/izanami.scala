  ...
  val format = new Format[NgIzanamiV1ProxyConfig] {
    ...
    override def reads(json: JsValue): JsResult[NgIzanamiV1ProxyConfig] = Try {
      NgIzanamiV1ProxyConfig(
        ...
        tls = json.select("tls").asOpt[JsValue].flatMap(NgTlsConfig.format.reads(_).asOpt).getOrElse(NgTlsConfig.default),
...
class NgIzanamiProxy extends NgRequestTransformer {
  ...
  private def getFeatures(ctx: NgTransformerRequestContext, config: NgIzanamiV1ProxyConfig)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
  ...
  private def getFeaturesWithBody(ctx: NgTransformerRequestContext, config: NgIzanamiV1ProxyConfig, body: Source[ByteString, _])(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
  ...
  private def getConfig(ctx: NgTransformerRequestContext, config: NgIzanamiV1ProxyConfig)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
    env.Ws
  ...
  override def transformRequest(ctx: NgTransformerRequestContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
    ...
    (ctx.request.method.toLowerCase, ctx.request.path) match {
      ...
      case ("post", path) if path == config.path + "/features" && config.featuresWithContextEnabled => getFeaturesWithBody(ctx, config, ctx.otoroshiRequest.body)
  ...
  val format = new Format[NgIzanamiV1CanaryRoutingConfig] {
    ...
    override def reads(json: JsValue): JsResult[NgIzanamiV1CanaryRoutingConfig] = Try {
      NgIzanamiV1CanaryRoutingConfig(
        routes = json.select("routes").asOpt[Seq[JsValue]].map(_.map(NgIzanamiV1CanaryRoutingConfigRoute.format.reads).collect { case JsSuccess(e, _) => e }).getOrElse(Seq.empty)
      )
    } match {
      case Failure(e) => JsError(e.getMessage)
      case Success(c) => JsSuccess(c)
...
class NgIzanamiV1Canary extends NgRequestTransformer {
  ...
  def fetchIzanamiRoutingConfig(config: NgIzanamiV1CanaryConfig, ctx: NgTransformerRequestContext)(implicit env: Env, ec: ExecutionContext): Future[NgIzanamiV1CanaryRoutingConfig] = {

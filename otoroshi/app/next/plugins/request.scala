...
case class NgDefaultRequestBodyConfig(body: ByteString, contentType: String, contentEncoding: Option[String]) extends NgPluginConfig {
...
object NgDefaultRequestBodyConfig {
  ...
  def from(ctx: NgTransformerRequestContext): NgDefaultRequestBodyConfig = {
    NgDefaultRequestBodyConfig(
      body = ctx.config.select("bodyStr").asOpt[String].filter(_.nonEmpty).map(ByteString.apply)
        .orElse(ctx.config.select("bodyBinary").asOpt[String].filter(_.nonEmpty).map(ByteString.apply).map(_.decodeBase64))
...
class NgDefaultRequestBody extends NgRequestTransformer {
  ...
  override def transformRequest(ctx: NgTransformerRequestContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, NgPluginHttpRequest]] = {
    if (hasNoRequestBody(ctx.request)) {
      ...
      ).applyOnWithOpt(config.contentEncoding) {
        case (hdrs, enc) => hdrs ++ Map("Content-Encoding" -> enc)
      }

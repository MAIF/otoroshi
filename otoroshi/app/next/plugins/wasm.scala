...
/*
class WasmConcurrentTestBackend extends NgBackendCall {

  private val logger                                       = Logger("otoroshi-plugins-wasm-concurrent-backend")
  override def useDelegates: Boolean                       = false
  override def multiInstance: Boolean                      = true
  override def core: Boolean                               = true
  override def name: String                                = "Wasm concurent test Backend"
  override def description: Option[String]                 = "This plugin can be used to use a wasm plugin as backend".some
  override def defaultConfigObject: Option[NgPluginConfig] = WasmConfig().some
  ...
  override def callBackend(
    ...
    ctx.wasmJson
      .flatMap(input => WasmUtils.execute(config, "call_backend", input.asObject.deepMerge(Json.obj("request" -> Json.obj("query" -> Json.obj("count" -> count)))), ctx.some, ctx.attrs.some))
      .map {
        case Right(output) =>
          val response =
            try {
          ...
          val ret_count = bodyJson.select("count").asLong
          if (ret_count == count) {
            bodyResponse(
              status = 201,
              headers = Map("Content-Type" -> "application/json"),
              body = Json.obj("message" -> "ok", "count" -> ret_count, "body" -> bodyJson).stringify.byteString.chunks(16 * 1024)
            )
          } else {
            val error = s"bad count, expected ${count}, found ${ret_count}"
            bodyResponse(
              status = 501,
              headers = Map("Content-Type" -> "application/json"),
              body = Json.obj("error" -> error, "body" -> bodyJson).stringify.byteString.chunks(16 * 1024)
            )
          }
        //bodyResponse(
        //  status = response.select("status").asOpt[Int].getOrElse(200),

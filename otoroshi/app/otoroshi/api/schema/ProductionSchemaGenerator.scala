...
class ProductionSchemaGenerator(
    ...
    private def handleEitherType(clazz: Class[_], context: SchemaContext): JValue = {
        ...
        val (leftSchema, rightSchema) = extractEitherTypes(clazz, context) match {
            case Some((left, right)) =>
                ...
                (leftObj, rightObj)
            case None =>
                (JObject(JField("title", JString("Left"))),
                 JObject(JField("title", JString("Right"))))
        }
    ...
    private def generateCollectionSchemaWithGenericInfo(collectionClass: Class[_], genericType: java.lang.reflect.Type, context: SchemaContext): JValue = {
    ...
    private def generateEnumerationSchemaFromField(genericType: java.lang.reflect.Type, context: SchemaContext): JValue = {
    ...
    private def handleADTType(clazz: Class[_], context: SchemaContext): JValue = {
        ...
        if (allCaseObjects) {
            ...
            val values = subtypes.map { subtype =>
            ...
            JObject(List(
                ...
                JField("enum", JArray(values.map(JString)))
            ))
        } else {
                ...
                val baseSchema = if (classOf[Map[_, _]].isAssignableFrom(fieldClass)) {
                    field.name match {
                        case "indexedData" =>
                            ...
                            val keyValueSchema = JObject(List(
                                JField("type", JString("object")),
                                JField("properties", JObject(List(
                                    JField("key", JObject()),
    ...
    private def extractCompanionDefaults(clazz: Class[_]): Map[String, Any] = {
        ...
        Option(defaultValueCache.getIfPresent(cacheKey)).getOrElse {
            val defaults = Try {
                ...
                val fieldNames = clazz.getDeclaredFields.filterNot { f =>
                    f.isSynthetic || java.lang.reflect.Modifier.isStatic(f.getModifiers)

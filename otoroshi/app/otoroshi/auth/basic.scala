...
object WebAuthnDetails {
  def fmt: Format[WebAuthnDetails] =
...
object BasicAuthModuleConfig extends FromJson[AuthModuleConfig] {
  ...
  val _fmt: Format[BasicAuthModuleConfig] = new Format[BasicAuthModuleConfig] {
    ...
    Try {
      Right(
        BasicAuthModuleConfig(
          ...
          users = (json \ "users").asOpt(using Reads.seq(using BasicAuthUser.fmt)).getOrElse(Seq.empty[BasicAuthUser]),

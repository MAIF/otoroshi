...
object LdapAuthModuleConfig extends FromJson[AuthModuleConfig] {
  ...
  val _fmt: Format[LdapAuthModuleConfig] = new Format[LdapAuthModuleConfig] {
...
object GroupRights {
  def _fmt: Format[GroupRights] = new Format[GroupRights] {
...
object GroupFilter {
  def _fmt: Format[GroupFilter] = new Format[GroupFilter] {
...
case class LdapAuthModuleConfig(
  ...
  override def asJson =
    location.jsonWithKey ++ Json.obj(
      ...
      "rightsOverride"                -> new JsObject(rightsOverride.view.mapValues(_.json).toMap),
      "dataOverride"                  -> JsObject(dataOverride),
      "allowedUsers"                  -> allowedUsers,
      "deniedUsers"                   -> deniedUsers,
      "groupRights"                   -> new JsObject(groupRights.view.mapValues(GroupRights._fmt.writes).toMap),
      "adminEntityValidatorsOverride" -> new JsObject(adminEntityValidatorsOverride.view.mapValues { o =>
        new JsObject(o.view.mapValues(v => JsArray(v.map(_.json))).toMap)
      }.toMap)
    )
                ...
                while (all.hasMore) {
                  ...
                  if (regexes.isEmpty || regexes.exists(_.matches(name))) {
                    val value = if (next.size() > 1) {
                      JsArray((0 until next.size()).map(idx => JsString(next.get(idx).toString)))
                    } else if (next.size() == 1) {

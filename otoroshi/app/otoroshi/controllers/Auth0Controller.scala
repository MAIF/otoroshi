              ...
              case Some(user) =>
                ...
                req
                  ...
                  case redirectTo                  =>
                    ...
                    val url               = new java.net.URI(redirectTo).toURL
          ...
          .map { paUser =>
            ...
            ctx.request.body.asFormUrlEncoded match {
              case Some(body) if body.get("RelayState").exists(_.nonEmpty) =>
                ...
                // val params      = queryParams.groupBy(_._1).view.mapValues(_.map(_._2).head)
                val params: Map[String, String] = {
                  try {
                    val decoded =
                      JWT.require(Algorithm.HMAC512(env.otoroshiSecret)).build().verify(body("RelayState").head)
                    decoded.getClaims.asScala.view.mapValues(_.asString()).filter(_._2 != null).toMap
                  } catch {
                    case t: Throwable =>
                ...
                val url  = java.net.URI.create(redirectTo).toURL
                val host = url.getHost

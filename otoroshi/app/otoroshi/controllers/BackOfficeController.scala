  ...
  def getEnv() =
    BackOfficeActionAuth.async { ctx =>
      ...
      for {
        ...
        val changePassword            = users.exists { user =>
                ...
                val apps = cleverapps.value
                  .map { app =>
                    ...
                    val hosts: Seq[String] =
                      (app \ "vhosts").as[JsArray].value.map(vhost => (vhost \ "fqdn").as[String]).toSeq
                    val preferedHost       =
  ...
  def mostCalledServices() =
    BackOfficeActionAuth.async { ctx =>
      ...
      env.datastores.serviceDescriptorDataStore.findAll().flatMap { services =>
        ...
        items.sortWith(_.rate > _.rate).slice(paginationPosition, paginationPosition + paginationPageSize)
      } map { items =>
        items.map { i =>
    ...
    BackOfficeActionAuth.async { ctx =>
      ctx.checkRights(SuperAdminOnly) {
        ...
        env.datastores.auditDataStore.findAllRaw().map { elems =>
          val filtered = elems.slice(paginationPosition, paginationPosition + paginationPageSize)
          Ok.chunked(
            Source
              .single(ByteString("["))
              .concat(

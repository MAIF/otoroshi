...
case class JwtInjection(
  ...
  def asJson: JsValue =
    Json.obj(
      ...
      "additionalHeaders" -> new JsObject(this.additionalHeaders.view.mapValues(JsString.apply).toMap),
      "removeHeaders"     -> JsArray(this.removeHeaders.map(JsString.apply)),
      "additionalCookies" -> new JsObject(this.additionalCookies.view.mapValues(JsString.apply).toMap),
...
case class JWKSAlgoSettings(
  ...
  def fetchJWKS(alg: String, kid: String, oldStop: Long, oldKeys: Map[String, com.nimbusds.jose.jwk.JWK])(implicit
    ...
    implicit val s: Scheduler = env.otoroshiScheduler
            ...
            algoSettings.asAlgorithm(OutputMode) match {
              ...
              case Some(outputAlgorithm) =>
                val moreCtx           = Map(
                  ...
                  "iat" -> s"${Math.floor(System.currentTimeMillis().toDouble / 1000L).toLong}",
                  "nbf" -> s"${Math.floor(System.currentTimeMillis().toDouble / 1000L).toLong}",
                  "iss" -> "Otoroshi",
                  "exp" -> s"${Math.floor((System.currentTimeMillis() + 60000L).toDouble / 1000L).toLong}",
...
object GlobalJwtVerifier extends FromJson[GlobalJwtVerifier] {
  ...
  val _fmt: Format[GlobalJwtVerifier] = new Format[GlobalJwtVerifier] {

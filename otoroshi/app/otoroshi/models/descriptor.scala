...
import otoroshi.utils.{RegexPool, ReplaceAllWith, TypedMap}
...
object ApiDescriptor {
  given format: OFormat[ApiDescriptor] = Json.format[ApiDescriptor]
}
...
object BaseQuotas {
  given format: OFormat[BaseQuotas] = Json.format[BaseQuotas]
...
object BestResponseTime extends LoadBalancing {
  ...
  override def select(
    ...
    val existing                 = responseTimes.toSeq.filter(t => keys.contains(t._1))
...
case class WeightedBestResponseTime(ratio: Double) extends LoadBalancing {
  ...
  override def select(
    ...
    val keys                     = targets.map(t => s"$descId-${t.asKey}")
    val existing                 = BestResponseTime.responseTimes.toSeq.filter(t => keys.contains(t._1))
...
object IpFiltering {
  given format: OFormat[IpFiltering] = Json.format[IpFiltering]
...
object HealthCheck {
  given format: Format[HealthCheck] = new Format[HealthCheck] {
...
object CustomTimeouts {
  ...
  given format: Format[CustomTimeouts] = new Format[CustomTimeouts] {
...
object ClientConfig {
  ...
  given format: Format[ClientConfig] = new Format[ClientConfig] {
      ...
      Try {
        ClientConfig(
          ...
          customTimeouts = (json \ "customTimeouts")
            .asOpt[JsArray]
            .map(_.value.map(e => CustomTimeouts.format.reads(e).get).toSeq)
...
object Canary {
  ...
  given format: Format[Canary] = new Format[Canary] {
      ...
      Try {
        Canary(
          ...
          targets = (json \ "targets")
            .asOpt[JsArray]
            .map(_.value.map(e => Target.format.reads(e).get).toSeq)
...
object RedirectionSettings {
  ...
  given format: Format[RedirectionSettings] = new Format[RedirectionSettings] {
    ...
    override def reads(json: JsValue): JsResult[ServiceDescriptor] =
      Try {
        ServiceDescriptor(
          ...
          api = (json \ "api").asOpt(using ApiDescriptor.format).getOrElse(ApiDescriptor(exposeApi = false, None)),
          healthCheck = (json \ "healthCheck").asOpt(using HealthCheck.format).getOrElse(HealthCheck(enabled = false, "/")),
...
trait ServiceDescriptorDataStore extends BasicStore[ServiceDescriptor] {
  ...
  def sortServices(
    ...
    val sersWithMatchingRoot    = filtered1.filter(_.allPaths.nonEmpty).sortWith { case (a, b) =>
      val aMatchedSize = matched.get(a.id).map(_.length).getOrElse(0)
      val bMatchedSize = matched.get(b.id).map(_.length).getOrElse(0)
    ...
    FastFuture
      ...
      .map { s =>
        val allSers = s.filter(_._2).map(_._1)
        val res1    = if (filtered.nonEmpty && filtered.size > allSers.size && allSers.isEmpty) {
          // let apikey check in handler produce an Unauthorized response instead of service not found
          Seq(filtered.last)
        } else {
          allSers

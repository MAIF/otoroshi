      ...
      Try {
        NgClientConfig(
          ...
          customTimeouts = (json \ "custom_timeouts")
            .asOpt[JsArray]
            .map(_.value.map(e => NgCustomTimeouts.format.reads(e).get).toSeq)
            .getOrElse(Seq.empty)
        )
      } match {
        case Failure(e) => JsError(e.getMessage())
...
object NgBackend {
  def empty: NgBackend                            = NgBackend(Seq.empty, "/", rewrite = false, RoundRobin, None, NgClientConfig())
...
object NgMinimalBackend {
  def empty: NgMinimalBackend                            = NgMinimalBackend(Seq.empty, "/", rewrite = false, RoundRobin)

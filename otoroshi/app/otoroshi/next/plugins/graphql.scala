...
class GraphQLBackend extends NgBackendCall {
  ...
  def builder(
      ...
      delegates: () => Future[Either[NgProxyEngineError, BackendCallResponse]],
      body: JsObject
  )(using env: Env, ec: ExecutionContext, mat: Materializer): ResolverBasedAstSchemaBuilder[Unit] =
...
//    DirectiveResolver(wasmDirective, resolve = c => wasmDirectiveResolver(c, ctx)), // fixme with pekko update
      ...
      .map { resp =>
        if (resp.status == 200) {
          resp.json.atPath(c.arg(responsePathArg).getOrElse("$")).asOpt[JsValue].getOrElse(JsNull) match {
            case JsArray(value) =>
              val res = sliceArrayWithArgs(value.toIndexedSeq, c)
              res.map {
                case v: JsObject  => v
                case JsString(v)  => v
                case JsNumber(v)  => v
    ...
    c.arg(jsonPathArg) match {
      case Some(path) =>
        c.ctx.field.toAst.fieldType match {
          case ListType(_, __) =>
            sliceArrayWithArgs(
              JsonPathUtils.getAtPolyJson(data, path).getOrElse(Json.arr()).asInstanceOf[JsArray].value.toIndexedSeq,
              c
            )
  ...
  private def mockDirectiveResolver(c: AstDirectiveContext[Unit], rawConfig: Option[JsObject])(using
      env: Env
  ): Object = {
    rawConfig match {
  ...
  def replaceTermsInUrl(c: AstDirectiveContext[Unit])(using env: Env): String = {
    val queryArgs = c.ctx.args.raw.map {
      case (str, Some(v)) => (str, String.valueOf(v))
      case (k, v)         => (k, String.valueOf(v))
    }
    ...
    val ctx: Map[String, String] = (extractedField ++ paramsArgs ++ fieldsFromValue).toMap
          ...
          bodyToJson(value.response.body).map(body => {
            if (value.status > 299) {
              ...
              ((body \ "json").as[JsObject] \ "data").select(c.ctx.field.toAst.name).asOpt[JsArray] match {
                case Some(value) => sliceArrayWithArgs(value.value.toIndexedSeq, c)
  ...
  def bodyToJson(source: Source[ByteString, ?])(using mat: Materializer, ec: ExecutionContext): Future[JsObject] =
    source
      .runFold(ByteString.empty)(_ ++ _)
      .map { rawBody =>
        {

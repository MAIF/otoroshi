...
class OAuth1Caller extends NgRequestTransformer {
  ...
  def getOauthParams(
    ...
    Map(
      ...
      OAuth1Caller.Keys.timestamp       -> s"${Math.floor(System.currentTimeMillis().toDouble / 1000).toInt}",
  ...
  def getToken(key: String, config: OAuth2CallerConfig)(using
    ...
    env.MtlsWs
      ...
      .applyOnIf(config.kind == OAuth2Kind.PasswordWithBasicAuth) { b =>
        b.addHttpHeaders("Authorization" -> s"Basic ${Base64.getEncoder
          .encodeToString(s"${config.clientId}:${config.clientSecret}".getBytes(StandardCharsets.UTF_8))}")
      }
      .withBody(body)
      .execute()
      .flatMap { resp =>

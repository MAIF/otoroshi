...
class TailscaleLocalApiClient(env: Env) {
  ...
  private val client: HttpClient = if (!doesMacOSDomainSocketExists) {
    ...
    HttpClient
      ...
      .runOn {
        if (OS.isMac) {
          DefaultLoopResourcesHelper.getKQueueLoop("tailscale-group", 2, daemon = true)
        } else if (OS.isLinux) {
          DefaultLoopResourcesHelper.getEpollLoop("tailscale-group", 2, daemon = true)
        } else {
          DefaultLoopResourcesHelper.getDefaultLoop("tailscale-group", 2, daemon = true)
        }
      }

...
case class OpenApiGeneratorConfig(filePath: String, raw: JsValue) {
  ...
  def write(): Unit = {
    val config = Json.obj(
      ...
      // "old_descriptions" -> JsObject(old_descriptions.view.mapValues(JsString.apply)),
      // "old_examples" -> JsObject(old_examples.view.mapValues(JsString.apply)),
    ...
    val descs       = descriptions.view
      ...
      .mkString(",\n")
    // val olddescs = old_descriptions.view.mapValues(JsString.apply).toSeq.sortWith((a, b) => a._1.compareTo(b._1) < 0).map(t => s"    ${JsString(t._1).stringify}: ${t._2.stringify}").mkString(",\n")

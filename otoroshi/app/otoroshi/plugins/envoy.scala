...
class EnvoyControlPlane extends RequestTransformer {
  ...
  def handleListenerDiscovery(
    ...
    def httpsListener(id: String, port: Int, services: Seq[ServiceDescriptor], certificates: Seq[Cert]): JsObject = {
      ...
      val chains = services.flatMap(s => s.allHosts.map(h => (h, s))).groupBy(_._1).view.mapValues(_.map(_._2)).map {
        case (host, servs) =>
          val certs =
      ...
      def filterChain(t: (String, (Seq[Cert], Seq[ServiceDescriptor]))): Option[JsObject] =
        t match {
          case (host, (certs, servs)) =>
    ...
    env.datastores.certificatesDataStore.findAll().flatMap { __certs =>
      env.datastores.serviceDescriptorDataStore.findAll().map { _services =>
        ...
        val listeners = Seq(
          httpListener("http", 10080 /*env.port*/, services),
          httpsListener("https", 10443 /*env.httpsPort*/, services, certs)
        )
        Results.Ok(
          Json.obj(
            "version_info" -> "1.0",

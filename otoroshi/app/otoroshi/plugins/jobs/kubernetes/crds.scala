...
class ClientSupport(val client: KubernetesClient, logger: Logger)(using ec: ExecutionContext, env: Env) {
  ...
  private[kubernetes] def crdsFetchExtensionResources(
    ...
    val futures = resources.toSeq.map { case (resource, values) =>
      () =>
        client
...
object KubernetesCRDsJob {
  ...
  def importCRDEntities(conf: KubernetesConfig, attrs: TypedMap, clientSupport: ClientSupport, ctx: CRDContext)(using
      env: Env,
      ec: ExecutionContext
  ): Future[Unit] = {
    given mat: Materializer = env.otoroshiMaterializer
      ...
      .mapAsync(1) { case (namespace, name, apikey) =>
        clientSupport.client.fetchSecret(namespace, name).flatMap {
          ...
          case Some(secret) =>
            ...
            updatedSecrets.updateAndGet(seq => seq :+ (namespace, name))
            if (
              kind.contains("otoroshi.io/apikey-secret") && (!clientId.contains(apikey.clientId) || !clientSecret
                .contains(apikey.clientSecret))
            ) {
              ...
              if (!kind.contains("otoroshi.io/apikey-secret")) {
                logger.error(
                  s"unable to export apikey ${namespace}/${name} as secret, because secret already exists and is not of type 'otoroshi.io/apikey-secret'"
                )
              }
              ().future
            }
  ...
  def syncCRDs(_conf: KubernetesConfig, attrs: TypedMap, jobRunning: => Boolean)(using
      env: Env,
      ec: ExecutionContext
  ): Future[Unit] =

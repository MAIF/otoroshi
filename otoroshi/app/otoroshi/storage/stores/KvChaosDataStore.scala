...
class KvChaosDataStore(redisCli: RedisLike, _env: Env) extends ChaosDataStore {
  ...
  override def serviceAlreadyOutage(serviceId: String)(using ec: ExecutionContext, env: Env): Future[Boolean] = {
    redisCli.get(s"${env.storageRoot}:outage:bydesc:until:$serviceId").map(_.isDefined)
  }

  override def serviceOutages(serviceId: String)(using ec: ExecutionContext, env: Env): Future[Int] = {
    redisCli.get(s"${env.storageRoot}:outage:bydesc:counter:$serviceId").map(_.map(_.utf8String.toInt).getOrElse(0))
  }

  override def groupOutages(groupId: String)(using ec: ExecutionContext, env: Env): Future[Int] = {
  ...
  )(using ec: ExecutionContext, env: Env): Future[FiniteDuration] = {
  ...
  override def resetOutages()(using ec: ExecutionContext, env: Env): Future[Unit] = {
    for {
  ...
  override def startSnowMonkey()(using ec: ExecutionContext, env: Env): Future[Unit] = {
    for {
  ...
  override def stopSnowMonkey()(using ec: ExecutionContext, env: Env): Future[Unit] = {
    for {
      _ <- env.datastores.chaosDataStore.resetOutages()
      c <- env.datastores.globalConfigDataStore.singleton()

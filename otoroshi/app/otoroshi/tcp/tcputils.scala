...
object TcpUtils {
  ...
  val domainNamePattern: Pattern                                                                                                = Pattern.compile("(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]")
  ...
  def bindTlsWithSSLEngineAndSNI(
    ...
    val tcp = Tcp()
    tcp.bind(interface, port, backlog, options, halfClose = true, idleTimeout).map { incomingConnection =>

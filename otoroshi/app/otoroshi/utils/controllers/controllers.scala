...
trait AdminApiHelper {
  ...
  def fetchWithPaginationAndFiltering[Entity, Error](
    ...
    val filters            = ctx.request.queryString.view
      .mapValues(_.last)
      .toMap
      .collect {
...
trait EntityHelper[Entity <: EntityLocationSupport, Error] {
  ...
  // def canRead[A](ctx: ApiActionContext[A])(entity: Entity)(using env: Env): Boolean = ctx.canUserRead(entity)
  // def canWrite[A](ctx: ApiActionContext[A])(entity: Entity)(using env: Env): Boolean = ctx.canUserWrite(entity)
  // def canReadWrite[A](ctx: ApiActionContext[A])(entity: Entity)(using env: Env): Boolean = ctx.canUserRead(entity) && ctx.canUserWrite(entity)
...
trait BulkHelper[Entity <: EntityLocationSupport, Error] extends EntityHelper[Entity, Error] {
  ...
  def bulkCreate(ctx: ApiActionContext[Source[ByteString, ?]]): Future[Result] = {
    ...
    given implEnv: Env             = env
    given implEc: ExecutionContext = env.otoroshiExecutionContext
    given implMat: Materializer    = env.otoroshiMaterializer
...
trait BulkControllerHelper[Entity <: EntityLocationSupport, Error] extends BulkHelper[Entity, Error] {
  ...
  def bulkUpdateAction(): Action[Source[ByteString, ?]] = ApiAction.async(sourceBodyParser) { ctx => bulkUpdate(ctx) }
  def bulkCreateAction(): Action[Source[ByteString, ?]] = ApiAction.async(sourceBodyParser) { ctx => bulkCreate(ctx) }
  def bulkPatchAction(): Action[Source[ByteString, ?]]  = ApiAction.async(sourceBodyParser) { ctx => bulkPatch(ctx) }
  def bulkDeleteAction(): Action[Source[ByteString, ?]] = ApiAction.async(sourceBodyParser) { ctx => bulkDelete(ctx) }
}

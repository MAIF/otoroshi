...
case class GzipConfig(
  ...
  private def isNotAlreadyCompressed(header: ResponseHeader) = !header.headers.contains(CONTENT_ENCODING)
...
object GzipFlow {
  ...
  def gzipDecompress(bufferSize: Int = 512, max: Int = 64 * 1024): Flow[ByteString, ByteString, ?] = {

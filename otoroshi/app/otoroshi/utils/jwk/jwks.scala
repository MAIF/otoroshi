...
object JWKSHelper {
  ...
  def jwks(req: RequestHeader, certIds: Seq[String], includeExposed: Boolean, includeAlg: Boolean, rsaAlgorithms: Seq[com.nimbusds.jose.Algorithm], esAlgorithms: Seq[com.nimbusds.jose.Algorithm])(using
      ec: ExecutionContext,
      env: Env
          ...
          .flatMap(c =>
            Try((c.id, c.cryptoKeyPair.getPublic))
              .seffectOnWithPredicate(t => t.isFailure)(t =>
                t.asInstanceOf[Failure[(String, PublicKey)]].exception.printStackTrace()
              )
              .toOption
          )
          .flatMap {

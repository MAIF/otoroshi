...
object Match {
  ...
  private def matchesOperator(operator: JsObject, key: String, source: JsValue): Boolean = {
    operator.value.head match {
      ...
      case ("$in", JsArray(value))           => value.exists(singleMatches(source.select(key).as[JsValue], literalMatch = true))
      case ("$nin", JsArray(value))          => !value.exists(singleMatches(source.select(key).as[JsValue], literalMatch = true))
        ...
        case (key, o @ JsObject(_)) if Operator.isOperator(o) =>
          o.value.head match {
            ...
            case ("$includeAllKeysMatching", JsArray(values)) =>
              ...
              dest = JsObject(dest.value.filter { case (k, _) =>
                strValues.exists {
            ...
            case ("$excludeAllKeysMatching", JsArray(values)) =>
              val strValues = values.map(_.asString)
              dest = JsObject(dest.value.filterNot { case (k, _) =>
                strValues.exists {
                  case str if str.startsWith("StartsWith(") =>
                    val searching = str.substring(11).init
                    k.startsWith(searching)

        ...
        .map { response =>
          ...
          if (successPredicate.check(ctx.json)) {
            if (successStatuses.isEmpty) {
              ctx.responses.put(name, ctx.responses(name).as[JsObject] ++ Json.obj("success" -> true))
              WorkFlowResult.WorkFlowSuccess(this)
            } else if (successStatuses.contains(response.status)) {
              ctx.responses.put(name, ctx.responses(name).as[JsObject] ++ Json.obj("success" -> true))
              WorkFlowResult.WorkFlowSuccess(this)
            } else {
              ctx.responses.put(name, ctx.responses(name).as[JsObject] ++ Json.obj("success" -> false))

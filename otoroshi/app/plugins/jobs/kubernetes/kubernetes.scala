          ...
          Source(ingresses.toList)
            ...
            env.datastores.serviceDescriptorDataStore.findAll().flatMap { services =>
              val toDelete = services.filter { service =>
                service.metadata.get("otoroshi-service-provider").contains("kubernetes-ingresss")
              }.map { service =>
                    ...
                    env.datastores.serviceDescriptorDataStore.findById(id).map {
                      ...
                      case (action, desc) =>
                        ...
                        val newDesc = desc.copy(
                          ...
                          publicPatterns = Seq("/.*"),
                          metadata = Map(
                            "otoroshi-service-provider" -> "kubernetes-ingress",
...
class KubernetesClient(val config: KubernetesConfig, env: Env) {
  ...
  private def customizeServiceDescriptor(spec: JsValue, res: KubernetesOtoroshiResource): JsValue = {
    ...
    val meta = (inter2 \ "metadata").asOpt[JsObject].getOrElse(Json.obj()) ++ Json.obj(
      "otoroshi-service-provider" -> "kubernetes-crds",
  ...
  def crdsFetchServiceDescriptors(): Future[Seq[OtoResHolder[ServiceDescriptor]]] = fetchOtoroshiResources[ServiceDescriptor]("service-descriptors", ServiceDescriptor._fmt, customizeServiceDescriptor)
  def crdsFetchApiKeys(): Future[Seq[OtoResHolder[ApiKey]]] = fetchOtoroshiResources[ApiKey]("apikeys", ApiKey._fmt, customizeApikey)

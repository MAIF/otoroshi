...
class OAuth1CallerPlugin extends RequestTransformer {
  ...
  private def signRequest(verb: String, path: String, params: Seq[(String, String)], consumerSecret: String,
  ...
  def getOauthParams(consumerKey: String, consumerSecret: String, tokenSecret: Option[String] = None, signatureMethod: String): Map[String, String] =
    Map(
  ...
  private def authorization(
    ...
    val params: Seq[(String, String)] =
      (queryParams ++ oauthParams + (Keys.token -> oauthToken))
        .map { case (k, v) => (k, v) }.toSeq.filter { case (k, _) => k != "oauth_signature" }
  ...
  private def getOrElse(config: JsValue, metadata: Map[String, String], key: String): Either[Result, String] = {
    metadata.get(key) match {
      ...
      case None => (config \ key).asOpt[String] match {
  ...
  override def transformRequestWithCtx(ctx: TransformerRequestContext)(implicit env: Env, ec: ExecutionContext, mat: Materializer): Future[Either[Result, HttpRequest]] = {
    ...
    (consumerKey, consumerSecret, oauthToken, oauthTokenSecret) match {
      case (Right(key), Right(secret), Right(token), Right(tokenSecret)) =>
        ...
        Right(ctx.otoroshiRequest.copy(headers = ctx.otoroshiRequest.headers + ("Authorization" -> s"OAuth $params"))).future

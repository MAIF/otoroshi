...
class ServiceQuotas extends AccessValidator {
  ...
  private def totalCallsKey(name: String)(implicit env: Env): String   = s"${env.storageRoot}:plugins:services-public-quotas:global:$name"
  private def dailyQuotaKey(name: String)(implicit env: Env): String   = s"${env.storageRoot}:plugins:services-public-quotas:daily:$name"
  private def monthlyQuotaKey(name: String)(implicit env: Env): String = s"${env.storageRoot}:plugins:services-public-quotas:monthly:$name"
  private def throttlingKey(name: String)(implicit env: Env): String   = s"${env.storageRoot}:plugins:services-public-quotas:second:$name"

  private def updateQuotas(descriptor: ServiceDescriptor, qconf: ServiceQuotasConfig, increment: Long = 1L)(implicit ec: ExecutionContext, env: Env): Future[Unit] = {
    ...
    for {
      ...
      secTtl <- env.datastores.rawDataStore.pttl(throttlingKey(descriptor.id)).filter(_ > -1).recoverWith {
        case _ => env.datastores.rawDataStore.pexpire(throttlingKey(descriptor.id), env.throttlingWindow * 1000)
      }
  ...
  private def withingQuotas(descriptor: ServiceDescriptor, qconf: ServiceQuotasConfig)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
    for {
  ...
  private def withinThrottlingQuota(descriptor: ServiceDescriptor, qconf: ServiceQuotasConfig)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
  ...
  private def withinDailyQuota(descriptor: ServiceDescriptor, qconf: ServiceQuotasConfig)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
    env.datastores.rawDataStore.get(dailyQuotaKey(descriptor.id)).fast.map(_.map(_.utf8String.toLong).getOrElse(0L) < qconf.dailyQuota)

  private def withinMonthlyQuota(descriptor: ServiceDescriptor, qconf: ServiceQuotasConfig)(implicit ec: ExecutionContext, env: Env): Future[Boolean] =
    env.datastores.rawDataStore

        ...
        builder
          ...
          .map { resp =>
            ...
            GatewayEvent(
              ...
              headersOut = resp.headers.view.mapValues(_.last).toMap.toSeq.map(Header.apply),
              otoroshiHeadersIn = headers.map(Header.apply),
              otoroshiHeadersOut = resp.headers.view.mapValues(_.last).toMap.toSeq.map(Header.apply),
              extraInfos = None,

...
trait Pki {
  ...
  def genCsr(query: ByteString, caCert: Option[X509Certificate])(
      implicit ec: ExecutionContext
  ): Future[Either[String, GenCsrResponse]] = GenCsrQuery.fromJson(Json.parse(query.utf8String)) match {
    case Left(err) => Left(err).future
  ...
  def genSelfSignedCA(query: ByteString)(implicit ec: ExecutionContext,
                                         mat: Materializer): Future[Either[String, GenCertResponse]] =
    GenCsrQuery.fromJson(Json.parse(query.utf8String)) match {
      case Left(err) => Left(err).future
  ...
  // gencsr           generate a private key and a certificate request
  def genCsr(query: GenCsrQuery, caCert: Option[X509Certificate])(
      implicit ec: ExecutionContext
...
class BouncyCastlePki(generator: IdGenerator) extends Pki {
  ...
  // gencsr           generate a private key and a certificate request
  override def genCsr(query: GenCsrQuery, caCert: Option[X509Certificate])(
      implicit ec: ExecutionContext
  ): Future[Either[String, GenCsrResponse]] = {
              ...
              ca =>
                extensionsGenerator.addExtension(
                  ...
                  new AuthorityKeyIdentifier(
                    new GeneralNames(new GeneralName(new X509Name(ca.getSubjectX500Principal.getName))),
                    ca.getSerialNumber
                  )
              )
            )

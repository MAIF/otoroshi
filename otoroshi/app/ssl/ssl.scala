...
case class Cert(
  ...
  def save()(using ec: ExecutionContext, env: Env) = {
  ...
  def delete()(using ec: ExecutionContext, env: Env) = env.datastores.certificatesDataStore.delete(this)
  def exists()(using ec: ExecutionContext, env: Env) = env.datastores.certificatesDataStore.exists(this)
...
trait CertificateDataStore extends BasicStore[Cert] {
  ...
  def importInitialCerts(logger: Logger)(using env: Env, ec: ExecutionContext) = {
    importOneCert(
      "root CA certificate",
      env.configuration,
  ...
  /**
   * Local, in memory dedup of the generations in flight: several handshakes can reach the same not yet
   * existing autoCert domain at the same time, and each of them would otherwise run its own key generation
   * and save its own entity. The first caller owns the generation, the others share its future, so a herd
   * gets one certificate instead of one each. It holds per JVM: two instances of a cluster can still each
   * generate one, which the check against the certificates already in RAM closes once one of them is synced.
   */
  ...
  /**
   * Persists an auto generated certificate. A worker holds a local in memory datastore that is wiped at the
   * next state sync, so there the certificate is handed to the leader, which owns persistence: otherwise
   * every worker serves its own certificate for the domain and generates a new one as soon as its in memory
   * copy expires.
   *
   * The hand over to the leader is deliberately NOT awaited. This runs inside a handshake, behind a blocking
   * wait, and the call is a round trip with retries (worker.retries x worker.timeout): waiting for it would
   * let a slow or unreachable leader fail a handshake that has everything it needs. The certificate serves
   * the handshake in progress from `DynamicKeyManager.cache` and stays available from `autogenCerts`, so the
   * leader only has to catch up, in the background. `saveCertificate` logs and recovers its own failures.
   */
  private def persistAutoGeneratedCert(cert: Cert)(using env: Env, ec: ExecutionContext): Future[Unit] = {
    if (env.clusterConfig.mode.isWorker) {
      // held until the synced state carries it, so an unreachable leader does not turn into a new key pair
      // for the same domain every time the in memory copy would have expired
      DynamicSSLEngineProvider.pendingLeaderCerts
        .put(cert.id, DynamicSSLEngineProvider.PendingLeaderCert(cert, System.currentTimeMillis()))
      val _ = env.clusterAgent.saveCertificate(cert)
      FastFuture.successful(())
    } else {
      cert.save().map(_ => ())
    }
...
object DynamicSSLEngineProvider {
  ...
  // certificates a worker generated and could not hand over to the leader yet. No expiry, unlike
  // autogenCerts: on a worker nothing else holds them (its datastore is wiped at every state sync), so
  // letting them go while the leader is unreachable would have the next handshake generate another key pair
  // for the same domain, again and again. Entries leave when the synced state carries them, or by eviction.
  private[ssl] case class PendingLeaderCert(cert: Cert, lastAttempt: Long)
  private[ssl] val pendingLeaderCerts =
    Scaffeine().maximumSize(1000).build[String, PendingLeaderCert]()
  ...
  private def allUnrevokedCertMap: TrieMap[String, Cert] = {
    ...
    val pendingCerts   = pendingLeaderCerts.asMap().view.mapValues(_.cert).toMap
    new UnboundedTrieMap[String, Cert]().++=(datastoreCerts).++=(genCerts).++=(pendingCerts)
  }
  ...
  /**
   * Ticks the hand over of the certificates a worker generated: drops those the synced state now carries -
   * the leader accepted them - and pushes the others again, with a floor between two attempts. Called from
   * the certificate datastore sync loop, so it runs on every node but only does something on a worker.
   */
  def syncPendingLeaderCerts(env: Env): Unit = {
    if (env.clusterConfig.mode.isWorker && pendingLeaderCerts.estimatedSize() > 0L) {
      val now = System.currentTimeMillis()
      pendingLeaderCerts.asMap().foreach { case (id, pending) =>
        if (env.proxyState.certificate(id).isDefined) {
          pendingLeaderCerts.invalidate(id)
        } else if ((now - pending.lastAttempt) > env.autoCertLeaderRetryEvery.toMillis) {
          pendingLeaderCerts.put(id, pending.copy(lastAttempt = now))
          val _ = env.clusterAgent.saveCertificate(pending.cert)
        }
      }
    }
  }

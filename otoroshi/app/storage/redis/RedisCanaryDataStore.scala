...
class RedisCanaryDataStore(redisCli: RedisClientMasterSlaves, _env: Env) extends CanaryDataStore {

  lazy val logger = Logger("otoroshi-redis-canary-datastore")
  ...
  override def isCanary(serviceId: String, trackingId: String, traffic: Double)(implicit ec: ExecutionContext,
    ...
    for {
      ...
      fcanarycount   = redisCli.get(canaryCountKey(serviceId).key).fast.map(_.map(_.utf8String.toLong).getOrElse(0L))
      fstandardCount = redisCli.get(standardCountKey(serviceId).key).fast.map(_.map(_.utf8String.toLong).getOrElse(0L))
      ...
      isNowCanary <- if (!alreadyCanary && !alreadyStd && currentPercent < traffic) {
                      redisCli.sadd(canarySessionsKey(serviceId).key, trackingId).fast.flatMap { nbr =>
                        redisCli.incr(canaryCountKey(serviceId).key).fast.map { count =>
                      ...
                      redisCli.sadd(standardSessionsKey(serviceId).key, trackingId).fast.flatMap { nbr =>
                        redisCli.incr(standardCountKey(serviceId).key).fast.map { count =>
                          env.datastores.globalConfigDataStore.singleton().fast.map { config =>
                            env.statsd
  ...
  def canaryCampaign(serviceId: String)(implicit ec: ExecutionContext, env: Env): Future[ServiceCanaryCampaign] =
    for {
      canary   <- redisCli.get(canaryCountKey(serviceId).key).fast.map(_.map(_.utf8String.toLong).getOrElse(0L))
      standard <- redisCli.get(standardCountKey(serviceId).key).fast.map(_.map(_.utf8String.toLong).getOrElse(0L))
      _ <- env.datastores.globalConfigDataStore.singleton().fast.map { config =>

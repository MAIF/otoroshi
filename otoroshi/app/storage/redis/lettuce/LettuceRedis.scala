...
class LettuceRedisStandaloneAndSentinels(actorSystem: ActorSystem, client: RedisClient) extends LettuceRedis {
  ...
  override def health()(implicit ec: ExecutionContext): Future[DataStoreHealth] = redis.info().toScala.map(_ => Healthy).recover {
    case _ => Unreachable
  }
  ...
  override def mget(keys: String*): Future[Seq[Option[ByteString]]] = redis.mget(keys: _*).toScala.map(_.asScala.toSeq.map(v => if (v.hasValue) Option(v.getValue) else None))
  ...
  override def setBS(key: String, value: ByteString, exSeconds: Option[Long], pxMilliseconds: Option[Long]): Future[Boolean] = {
    exSeconds.map(v => SetArgs.Builder.ex(v)).orElse(
      pxMilliseconds.map(v => SetArgs.Builder.px(v))
    ) match {
      case None =>  redis.set(key, value).toScala.map {
        case "OK" => true
        case _ => false
      }
      case Some(args) => redis.set(key, value, args).toScala.map {
        case "OK" => true
        case _ => false
      }
    }
  ...
  override def incrby(key: String, increment: Long): Future[Long] = redis.incrby(key, increment).toScala.map(_.longValue())
  ...
  override def hsetBS(key: String, field: String, value: ByteString): Future[Boolean] = redis.hset(key, field, value).toScala.map(_.booleanValue())
  ...
  override def lpushLong(key: String, values: Long*): Future[Long] = lpushBS(key, values.map(v => ByteString(v.toString)): _*)

  override def lpushBS(key: String, values: ByteString*): Future[Long] = redis.lpush(key, values: _*).toScala.map(_.longValue())

  override def lrange(key: String, start: Long, stop: Long): Future[Seq[ByteString]] = redis.lrange(key, start, stop).toScala.map(_.asScala.toSeq)

  override def ltrim(key: String, start: Long, stop: Long): Future[Boolean] = redis.ltrim(key, start, stop).toScala.map {
    case "OK" => true
    case _ => false
  }
  ...
  override def expire(key: String, seconds: Int): Future[Boolean] = redis.expire(key, seconds).toScala.map(_.booleanValue())

  override def pexpire(key: String, milliseconds: Long): Future[Boolean] = redis.pexpire(key, milliseconds).toScala.map(_.booleanValue())

  override def sadd(key: String, members: String*): Future[Long] = saddBS(key, members.map(ByteString.apply): _*)

  override def saddBS(key: String, members: ByteString*): Future[Long] = redis.sadd(key, members: _*).toScala.map(_.longValue())

  override def sismember(key: String, member: String): Future[Boolean] = sismemberBS(key, ByteString(member))

  override def sismemberBS(key: String, member: ByteString): Future[Boolean] = redis.sismember(key, member).toScala.map(_.booleanValue())
  ...
  override def sremBS(key: String, members: ByteString*): Future[Long] = redis.srem(key, members: _*).toScala.map(_.longValue())

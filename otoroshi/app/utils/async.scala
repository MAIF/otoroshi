...
object AsyncUtils {
  ...
  def chainAsyncF[I, A](
      items: Seq[I]
  )(input: A)(f: Function2[I, A, Future[A]])(using ec: ExecutionContext): Future[A] = {
    val promise   = Promise[A]()
    var latest: A = input
  ...
  def chainAsyncFE[Err, I, A](
      items: Seq[I]
  )(input: A)(f: Function2[I, A, Future[Either[Err, A]]])(using ec: ExecutionContext): Future[Either[Err, A]] = {
    val promise   = Promise[Either[Err, A]]()
    var latest: A = input

...
object JWKSHelper {
  ...
  def jwks(req: RequestHeader, certIds: Seq[String])(implicit ec: ExecutionContext, env: Env): Future[Either[JsValue, JsValue]] = {
    if (req.method == "GET") {
      for {
        ...
        val exposedCerts = certs.filter(c => (c.exposed || ids.contains(c.id)) && c.notRevoked && c.notExpired).map(c => (c.id, c.cryptoKeyPair.getPublic)).flatMap {

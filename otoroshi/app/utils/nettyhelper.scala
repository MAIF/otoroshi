...
object NettyHelper {
  ...
  def newServerOp(nettyRequest: HttpRequest, nettyResponse: HttpResponse, ctx: ChannelHandlerContext): reactor.netty.http.server.HttpServerOperations = {
    ...
    val connectionInfo: ConnectionInfo = ConnectionInfo.from(ctx.channel(), nettyRequest, true, ctx.channel().remoteAddress(), (info, req) => info)
    val listener: ConnectionObserver = ???
    val compressionPredicate: BiPredicate[HttpServerRequest, HttpServerResponse] = (req: HttpServerRequest, res: HttpServerResponse) => false
    val mapHandle: BiFunction[_ >: Mono[Void], _ >: Connection, _ <: Mono[Void]] = (mono: Mono[Void], conn: Connection) => mono

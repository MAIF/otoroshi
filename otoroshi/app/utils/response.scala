...
object ResponseImplicits {
  ...
  implicit class EnhancedWSResponse(val response: WSResponse) extends AnyVal {
    ...
    private def buildCookies(headers: Map[String, scala.collection.Seq[String]], env: Env): scala.collection.Seq[WSCookie] = {
    ...
    def safeCookies(env: Env): scala.collection.Seq[WSCookie] = try {
      ...
      case t: Throwable if t.getMessage.contains("Cannot invoke \"play.shaded.ahc.io.netty.handler.codec.http.cookie.Cookie.name()\" because \"c\" is null") =>

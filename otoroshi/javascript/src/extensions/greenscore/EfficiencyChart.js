...
export const GREEN_SCORE_GRADES = {
  ...
  '#27ae60': (rank) =>
    rank < MAX_GREEN_SCORE_NOTE - Math.ceil(MAX_GREEN_SCORE_NOTE * 0.1) &&
    rank >= Math.ceil(MAX_GREEN_SCORE_NOTE * 0.5),
  '#f1c40f': (rank) =>
    rank < Math.ceil(MAX_GREEN_SCORE_NOTE * 0.9) && rank >= Math.ceil(MAX_GREEN_SCORE_NOTE * 0.5),
  '#d35400': (rank) =>
    rank < Math.ceil(MAX_GREEN_SCORE_NOTE * 0.5) && rank >= Math.ceil(MAX_GREEN_SCORE_NOTE * 0.1),
...
export const EfficiencyChart = (props) => {
  ...
  const getDataForADay = (day) => {
    ...
    setLoading(true);
    fetch(
      `/bo/api/proxy/api/extensions/green-score/efficiency/${props.group}/${props.route}?day=${day}`,
      {
        credentials: 'include',
        headers: {
          Accept: ' application/json',
  ...
  zeData.forEach((item) => {
    ...
    const matchingRange = hitRanges.find(
      (range) => hits >= range.rangeStart && hits <= range.rangeEnd
    );
    if (matchingRange) {
    ...
    <Section title={route.name}>
      <div style={{ display: 'flex', gap: '.5rem' }}>
        ...
        >
          <div className="d-flex justify-content-between">
            ...
            {!!day && (
              <div>
                ...
                <button className="btn btn-primary" onClick={() => setDay(undefined)}>
                  back
                </button>
              </div>
            )}
                ...
                onClick={() =>
                  setMode(
                    mode === visualizationMode.heat
                      ? visualizationMode.graphs
                      : visualizationMode.heat
                  )
                }
              >
            ...
            <>
                          ...
                          <Popover
                            ...
                            content={
                              <div className="d-flex flex-column">
                                ...
                                <div className={`info`}>
                                  usage time: {humanMillisecond(Math.round(avgDuration * hits))}
                                </div>
                                <div className={`info`}>
                                  unusage time:{' '}
                                  {humanMillisecond(
                                    60 * 60 * 1000 - Math.round(avgDuration * hits)
                                  )}
                                </div>
                              </div>
                            }
              ...
              <div className="d-flex flex-row justify-content-around">
                      ...
                      <Popover
                        ...
                        content={
                          <div className="d-flex flex-column">
                            ...
                            <div className={`info`}>
                              unusage time:{' '}
                              {humanMillisecond(60 * 60 * 1000 - Math.round(avgDuration * hits))}
                            </div>
                          </div>
                        }
                      >
                  ...
                  {!day &&
                    [1, 7, 13, 19, 24].map((idx) => {
                      return (
                        ...
                        >
                          {new Date(zeData[idx - 1]?.date).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </div>
                  ...
                  {!!day &&
                    [0, 6, 12, 18, 23].map((idx) => {
                      return (
                        ...
                        >
                          {new Date(zeData[idx * 6]?.date).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </div>
                ...
                <div className="heatmap-legend d-flex flex-column-reverse gap-1 justify-content-end align-items-baseline">
                  <div>low</div>
                  <Popover content={`no hit`}>
                    <div className={`heatpoint nul`} />
                  </Popover>
                  <Popover content={`from 1 to ${~~(maxHits * 0.05)} hits`}>
                    <div className={`heatpoint low`} />
                  </Popover>
                  <Popover content={`from ${~~(maxHits * 0.05) + 1} to ${~~(maxHits * 0.5)} hits`}>
                    <div className={`heatpoint medium-low`} />
                  </Popover>
                  <Popover content={`from ${~~(maxHits * 0.5) + 1} to ${~~(maxHits * 0.95)} hits`}>
                    <div className={`heatpoint medium-high`} />
                  </Popover>
                  <Popover content={`from ${~~(maxHits * 0.95) + 1} to ${maxHits} hits`}>
          ...
          {mode === visualizationMode.graphs && (
            <div style={{ maxHeight: 420, flex: 1, display: 'flex' }}>

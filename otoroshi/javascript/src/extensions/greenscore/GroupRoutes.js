...
export class GroupRoutes extends React.Component {
  ...
  render() {
    if (!(this.props.rootValue && this.props.rootValue.routes)) return null;
    ...
    return (
      <>
        ...
        <RoutesSelector
          allRoutes={allRoutes.filter((route) => !routes.find((r) => route.id === r.routeId))}
  ...
  return (
    <>
      ...
      >
        ...
        <span style={{ textTransform: 'uppercase', fontWeight: 600, color: 'var(--text)' }}>
          Action
        </span>
      </div>
      {routes.length === 0 && (
  ...
  const onRulesChange = (enabled, ruleId, allRulesOfTheSection) => {
    ...
    if (!statesOfCurrentDate) {
      statesOfCurrentDate = {
        date: today.getTime(),
        states:
          statesWithSection.length > 0
            ? statesWithSection[statesWithSection.length - 1].states
            : [],
      };
    }
    ...
    rest.onChange({
      ...route.rulesConfig,
      states: [...states.filter((f) => f.date !== today.getTime()), statesOfCurrentDate],
    });
  };
...
function RulesTables({ rest, states, onRulesChange }) {
  ...
  const getSectionStatus = (rules) =>
    rules.reduce((acc, rule) => acc && ruleIsEabled(rule.id), true);

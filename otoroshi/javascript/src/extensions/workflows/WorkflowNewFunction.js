...
export function WorkflowNewFunction(props) {
  ...
  const create = () => {
    ...
    if (workflow.functions[name]) {
      ...
      return client
        ...
        .then(() =>
          history.push(
            `/extensions/workflows/${params.workflowId}/functions/${newFunction.name}/designer`
          )
        );
    }
  };
  ...
  return (
    <Loader loading={!workflow}>
      <NgForm flow={flow} value={newFunction} schema={schema} onChange={setFunction} />
      <FeedbackButton
        type="success"
        className="d-flex ms-auto"
        onPress={create}
        text={params.functionName ? 'Save' : 'Create'}
      />
    </Loader>
  );
}

        ...
        >
            {[

                {
                    ...
                    "advice": "Ensures a fair distribution of requests but does not consider target load or response time."
                },
                {
                ...
                },
                {
                    ...
                    "advice": "Ensures the same client IP consistently hits the same target, which can help with session persistence."
                },
                {
                    "key": "Random",
                    "text": "Selects a target at random from the list of backend targets.",
                    "advice": "Simple and useful for even distribution when there are no major performance differences between targets."
                },
                {
                    "key": "Sticky",
                ...
                },
                {
                    ...
                    "advice": "Prioritizes targets with lower response times while considering configured weights."
                }].map(({ key, text, advice }) => (
                    <button
                        type="button"
                        ...
                        key={key}
                    >
                        <div style={{ fontWeight: 'bold', textAlign: 'left' }} className='py-2'>{key}</div>
                        <p className='m-0' style={{ textAlign: 'left', fontSize: '.9rem' }}>{text}</p>

                        <p className='m-0' style={{ textAlign: 'left', fontStyle: 'italic', fontSize: '.8rem' }}>{advice}</p>
                    </button>))}
        </div>
    </div>
}
...
export class EurekaTargetForm extends React.Component {
  ...
  componentDidMount() {
    findAllEurekaServers().then((servers) => {
      const p =
        this.props.route.plugins.find((p) => p.plugin === 'cp:otoroshi.next.plugins.EurekaTarget')
          .config || {};
      const inOtoroshiCoreEurekaServer = p.eureka_server;
      const inOtoroshiCoreEurekaApp = p.eureka_app;

      if (inOtoroshiCoreEurekaServer) this.fetchEurekaApps(inOtoroshiCoreEurekaServer);
  ...
  render() {
    return (
      <div className="target_information pt-3 mt-3">
        <div className="mb-2">
          <SelectInput
            label="Server"
        ...
        {this.state.eurekaApps.length > 0 && (
          <SelectInput
            label="Eureka app"

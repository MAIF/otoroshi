...
export class ExternalEurekaTargetForm extends React.Component {
  ...
  componentDidMount() {
    const config =
      this.props.route.plugins.find(
        (p) => p.plugin === 'cp:otoroshi.next.plugins.ExternalEurekaTarget'
      ).config || {};
  ...
  fetchFromUrl = () => {
    return getExternalEurekaServers(this.state.serviceUrlDefaultZone).then((apps) => {
      this.setState({

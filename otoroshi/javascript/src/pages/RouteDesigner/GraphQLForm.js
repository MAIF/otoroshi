...
const CreationButton = ({ confirm, text, placeholder }) => {
  ...
  if (onCreationField)
    return (
      <div className="d-flex-between" onClick={(e) => e.stopPropagation()}>
        <input
          type="text"
  ...
  return (
    <div onClick={() => setOpen(!open)} className="mb-1">
      ...
      {open &&
        (fields || []).map((field, i) => (
          <div
            className="d-flex-between element py-1 ps-1 pe-2"
            key={`field${i}`}
      ...
      {open && (
        <CreationButton text="New field" placeholder="New field name" confirm={createField} />
      )}
    </div>
  );
};

...
export default class MocksDesigner extends React.Component {
    ...
    showForm = (title, idx, elementName, render) => {
        ...
        window
            .popup(
                title,
                (ok, cancel) => render(ok, cancel, resource),
...
const GenerateEndpoint = ({ resources, confirm, cancel }) => {
    ...
    const onResourceChange = (name, resourceName) => {
        setEndpoints([
            ...
            { method: "DELETE", path: `/${name}s/:id`, enabled: true, resource: resourceName, status: 202 },
        ])
    }
    ...
    return <div className="designer p-3" style={{ background: "#373735", borderRadius: '4px' }}>
        ...
        <div className='row'>
            ...
            {endpoints.map((endpoint, i) => <div className='d-flex-between generated-endpoint'>
                ...
                <TextInput
                    flex={true}
                    value={endpoint.path}
                    onChange={path => setEndpoints(endpoints.map((e, j) => {
                        if (j === i)
                            return { ...e, path }
                        return e
                    }))}
                ...
                <div className='mb-3 ms-2'>
                    <SimpleBooleanInput
                        ...
                        onChange={enabled => setEndpoints(endpoints.map((e, j) => {
                            if (j === i)
                                return { ...e, enabled }
                            return e
                        }))} />
        ...
        return <div className="designer p-3" style={{ background: "#373735", borderRadius: '4px' }}>
            <div className='row'>
                ...
                <div className='d-flex-between mb-1'>
                    <SelectInput
                        flex={true}
                        possibleValues={["GET", "HEAD", "POST", "PUT", "DELETE", "CONNECT", "OPTIONS", "TRACE", "PATCH"]}
                        value={method}

...
function MoreActionsButton({ value, menu, history }) {
  return (
    <div className="ms-2 dropdown" style={{ height: '100%' }}>
      <button
        type="button"
...
function ManagerTitle({
  ...
  return (
    <div
      className="page-header d-flex align-item-center justify-content-between ms-0 mb-3"
      style={{
        paddingBottom: pathname === '/routes' ? 'initial' : 0,
      }}>
      <h4 className="flex" style={{ margin: 0 }}>
        {location.pathname.includes('route-compositions')
          ? 'Route compositions'
          : {
              flow: 'Designer',
  ...
  loadRoute = () => {
    ...
    if (routeId === 'new') {
      ...
      nextClient.fetch(nextClient.ENTITIES[this.props.entity.fetchName], routeId).then((res) => {
        if (!res.error) this.setState({ value: res }, this.updateSidebar);
      });
    }
  };

...
const Manager = ({ query, entity, ...props }) => {
  ...
  useEffect(() => {
    if (p.routeId === 'new') {
      ...
      nextClient.fetch(nextClient.ENTITIES[entity.fetchName], p.routeId).then((res) => {
          ...
          {!isCreation &&
            [
              ...
              },
              {
                ...
                title: 'Tester',
                disabledHelp:
                  'Your route is disabled. Navigate to the informations page to turn it',
              ...
              .filter((link) =>
                location.state?.routeFromService ? link.tab === 'Informations' : true
              )
              .map(
                ({
                  to,
                  ...
                  <HelpWrapper text={disabled ? disabledHelp : undefined} dataPlacement="bottom">
                    <div
                      className={`ms-2 ${moreClass ? moreClass : ''} ${dropdown ? 'dropdown' : ''}`}
                      style={{
                        opacity: hidden ? 0 : 1,
...
export default (props) => {
  ...
  useEffect(() => {
    if (pathname.endsWith('route-compositions')) props.setTitle('Routes compositions');
    else if (!query) props.setTitle('Routes');
  }, [search]);

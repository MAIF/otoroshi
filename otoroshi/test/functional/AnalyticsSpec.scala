...
import models.{ApiKey, ElasticAnalyticsConfig, RemainingQuotas, ServiceDescriptor}
...
class AnalyticsSpec()
  ...
  lazy val elasticUrl = "http://localhost:9201"
  ...
  s"Analytics API" should {
    ...
    "Global stats API" in {
      ...
      val (resp, status) = otoroshiApiCall("GET", s"/api/stats/global?from=${from.getMillis}&to=${now.getMillis}").futureValue

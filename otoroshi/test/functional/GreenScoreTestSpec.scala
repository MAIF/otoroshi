...
class GreenScoreTestSpec(name: String, configurationSpec: => Configuration) extends OtoroshiSpec {
  ...
  def fetch(
    ...
    val request = wsClient
      .url(if(isApiExtension) s"http://otoroshi-api.oto.tools:$port/apis/green-score.extensions.otoroshi.io/v1/green-scores$path" else
  ...
  private def MAX_SCORE_BY_SECTIONS = RulesManager.rules.foldLeft(Seq.empty[(String, Double)]) { case (acc, rule) =>
  ...
  s"Green score" should {
    ...
    "create group with rules" in {
      ...
      val created = fetch(method = "post", body = template.copy(
          id = "LESS_OLD_GROUP",
          routes = Seq(
            RouteRules(
              routeId = routes.head.id,
              rulesConfig = RulesRouteConfiguration(states = Seq(
                RuleStateRecord(
                  date = LESS_OLD_DATE,
